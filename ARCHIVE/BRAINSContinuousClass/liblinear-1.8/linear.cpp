...
public:
  ...
  double
  fun( double * w ) override;

  void
  grad( double * w, double * g ) override;

  void
  Hv( double * s, double * Hs ) override;

  int
  get_nr_variable( void ) override;
...
void
l2r_lr_fun::grad( double * w, double * g )
{
  int   i;
  int * y = prob->y;
  int   l = prob->l;
...
int
l2r_lr_fun::get_nr_variable( void )
{
  return prob->n;
}

void
l2r_lr_fun::Hv( double * s, double * Hs )
{
  int      i;
  int      l = prob->l;
  int      w_size = get_nr_variable();
...
void
l2r_lr_fun::Xv( double * v, double * _Xv )
{
  int             i;
  int             l = prob->l;
...
void
l2r_lr_fun::XTv( double * v, double * _XTv )
{
  int             i;
  int             l = prob->l;
  int             w_size = get_nr_variable();
...
void
l2r_l2_svc_fun::grad( double * w, double * g )
{
  int   i;
  int * y = prob->y;
  int   l = prob->l;
...
int
l2r_l2_svc_fun::get_nr_variable( void )
{
  return prob->n;
}

void
l2r_l2_svc_fun::Hv( double * s, double * Hs )
{
  int      i;
  int      l = prob->l;
  int      w_size = get_nr_variable();
...
void
l2r_l2_svc_fun::Xv( double * v, double * _Xv )
{
  int             i;
  int             l = prob->l;
  feature_node ** x = prob->x;
...
void
l2r_l2_svc_fun::subXv( double * v, double * _Xv )
{
  int             i;
  feature_node ** x = prob->x;
...
void
l2r_l2_svc_fun::subXTv( double * v, double * XTv )
{
  int             i;
  int             w_size = get_nr_variable();
  feature_node ** x = prob->x;
...
Solver_MCSVM_CS::Solver_MCSVM_CS( const problem * _prob, int _nr_class, double * weighted_C, double _eps,
                                  int _max_iter )
{
  this->w_size = _prob->n;
  this->l = _prob->l;
...
void
Solver_MCSVM_CS::solve_sub_problem( double A_i, int yi, double C_yi, int active_i, double * alpha_new )
{
  int      r;
  double * D;
...
bool
Solver_MCSVM_CS::be_shrunk( int i, int m, int yi, double alpha_i, double minG )
{
  double bound = 0;
...
void
Solver_MCSVM_CS::Solve( double * w )
{
  int      i, m, s;
  int      iter = 0;
...
static void
solve_l2r_l1l2_svc( const problem * prob, double * w, double eps, double Cp, double Cn, int solver_type )
{
  int      l = prob->l;
  int      w_size = prob->n;
  int      i, s, iter = 0;
...
//
//  min_\alpha  0.5(\alpha^T Q \alpha) + \sum \alpha_i log (\alpha_i) + (upper_bound_i - alpha_i) log (upper_bound_i -
// alpha_i) ,
//    s.t.      0 <= alpha_i <= upper_bound_i,
//
//  where Qij = yi yj xi^T xj and
...
void
solve_l2r_lr_dual( const problem * prob, double * w, double eps, double Cp, double Cn )
{
  int      l = prob->l;
  int      w_size = prob->n;
  int      i, s, iter = 0;
...
static void
solve_l1r_l2_svc( problem * prob_col, double * w, double eps, double Cp, double Cn )
{
  int l = prob_col->l;
...
static void
solve_l1r_lr( const problem * prob_col, double * w, double eps, double Cp, double Cn )
{
  int l = prob_col->l;
  int w_size = prob_col->n;
...
static void
transpose( const problem * prob, feature_node ** x_space_ret, problem * prob_col )
{
  int            i;
  int            l = prob->l;
  int            n = prob->n;
...
static void
group_classes( const problem * prob, int * nr_class_ret, int ** label_ret, int ** start_ret, int ** count_ret,
               int * perm )
{
  int   l = prob->l;
  int   max_nr_class = 16;
...
static void
train_one( const problem * prob, const parameter * param, double * w, double Cp, double Cn )
{
...
//
model *
train( const problem * prob, const parameter * param )
{
  int     i, j;
  int     l = prob->l;
  int     n = prob->n;
...
void
cross_validation( const problem * prob, const parameter * param, int nr_fold, int * target )
{
  int   i;
...
int
predict( const model * model_, const feature_node * x )
{
...
static const char * solver_type_table[] = { "L2R_LR",   "L2R_L2LOSS_SVC_DUAL", "L2R_L2LOSS_SVC", "L2R_L1LOSS_SVC_DUAL",
...
int
get_nr_feature( const model * model_ )
{
  return model_->nr_feature;
}

int
get_nr_class( const model * model_ )
{
  return model_->nr_class;
}

void
get_labels( const model * model_, int * label )
{
...
const char *
check_parameter( const problem *, const parameter * param )
{
  if ( param->eps <= 0 )
  {
    return "eps <= 0";
  ...
  if ( param->solver_type != L2R_LR && param->solver_type != L2R_L2LOSS_SVC_DUAL &&
       param->solver_type != L2R_L2LOSS_SVC && param->solver_type != L2R_L1LOSS_SVC_DUAL &&
       param->solver_type != MCSVM_CS && param->solver_type != L1R_L2LOSS_SVC && param->solver_type != L1R_LR &&
...
int
check_probability_model( const struct model * model_ )
{
  return model_->param.solver_type == L2R_LR || model_->param.solver_type == L2R_LR_DUAL ||
