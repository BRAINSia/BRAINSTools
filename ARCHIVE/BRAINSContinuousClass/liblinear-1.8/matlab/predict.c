...
void
read_sparse_instance( const mxArray * prhs, int index, struct feature_node * x, int feature_number, double bias )
{
  int      i, j, low, high;
  mwIndex *ir, *jc;
  double * samples;
...
void
do_predict( mxArray * plhs[], const mxArray * prhs[], struct model * model_, const int predict_probability_flag )
{
...
exit_with_help()
{
  mexPrintf( "Usage: [predicted_label, accuracy, decision_values/prob_estimates] = predict(testing_label_vector, "
             "testing_instance_matrix, model, 'liblinear_options','col')\n"
             "liblinear_options:\n"
