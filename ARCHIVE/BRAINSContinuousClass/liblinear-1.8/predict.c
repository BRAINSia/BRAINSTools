...
void
do_predict(FILE * input, FILE * output, struct model * model_)
{
  int correct = 0;
  int total = 0;
  int tPos = 0;
  ...
  while (readline(input) != NULL)
  {
    ...
    else if (predict_label != target_label && predict_label < 0)
    {
      ++fNeg;
    }
    else if (predict_label != target_label && predict_label > 0)
  ...
  printf("Sensitivity: = %g%% \n", (double)tPos / (tPos + fNeg) * 100);
  printf("Specificity: = %g%% \n", (double)tNeg / (tNeg + fPos) * 100);
  if (flag_predict_probability)
    free(prob_estimates);
}
...
exit_with_help()
{
  printf("Usage: predict [options] test_file model_file output_file\n"
         "options:\n"
