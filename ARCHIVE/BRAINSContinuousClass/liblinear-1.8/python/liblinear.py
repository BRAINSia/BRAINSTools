...
if find_library("linear"):
    ...
    if sys.platform == "win32":
        liblinear = CDLL(
            os.path.join(os.path.dirname(__file__), "../windows/liblinear.dll")
        )
    else:
        liblinear = CDLL(os.path.join(os.path.dirname(__file__), "../liblinear.so.1"))
...
fillprototype(
    ...
    None,
    [POINTER(problem), POINTER(parameter), c_int, POINTER(c_int)],
)

fillprototype(
    liblinear.predict_values,
    c_int,
...
fillprototype(
    liblinear.check_parameter, c_char_p, [POINTER(problem), POINTER(parameter)]
)
