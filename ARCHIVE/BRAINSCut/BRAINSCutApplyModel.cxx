...
BRAINSCutApplyModel ::ApplyOnSubject(DataSet & subject)
{
  ...
  while (roiIDsOrderNumber < this->m_myDataHandler->GetROIIDsInOrder().size())
  {
    ...
    if (roiDataSet->GetAttribute<StringValue>("GenerateVector") == "true")
    {
      ...
      if (!m_computeSSE)
      {
        PredictROI(roiInputVector, predictedOutputVector, roiIDsOrderNumber, inputVectorGenerator.GetInputVectorSize());
                                 ...
                                 const std::string &         roiReferenceFilename)
{
  ...
  for (PredictValueMapType::const_iterator it = predictedOutputVector.begin(); it != predictedOutputVector.end(); ++it)
  {
...
void
BRAINSCutApplyModel ::WriteLabelMapToBinaryImages(const DataSet & subject, const LabelImagePointerType & labelMapImage)
{
  try
  {
...
BRAINSCutApplyModel ::AmbiguousCountLabel(LabelImagePointerType & ambiguousMap,
...
LabelImagePointerType
BRAINSCutApplyModel ::CombineLabel(LabelImagePointerType & resultLabel,
                                   LabelImagePointerType & currentLabel,
...
inline void
BRAINSCutApplyModel ::PredictROI(InputVectorMapType &  roiInputFeatureVector,
                                 ...
                                 const unsigned int    inputVectorSize) const
{
  ...
  for (InputVectorMapType::iterator it = roiInputFeatureVector.begin(); it != roiInputFeatureVector.end(); ++it)
  {
    ...
    if (m_method == "ANN")
    {
      ...
      resultOutputVector.insert(std::pair<hashKeyType, scalarType>((it->first), openCVOutput.at<float>(0, roiNumber)));
