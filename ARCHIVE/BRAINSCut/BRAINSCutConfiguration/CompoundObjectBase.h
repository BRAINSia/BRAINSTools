...
class CompoundObjectBase : public ProcessObjectBase
{
private:
...
public:
  ...
  void
  Add(ProcessObjectBase * toAdd, const char * name)
  {
  ...
  void
  Add(ProcessObjectBase * toAdd, const std::string & name)
  {
  ...
  CollectAttValues(const char * attributeName) const
  {
    ...
    for (it = this->begin(); it != this->end(); ++it)
    {
      ...
      const std::string val(current->GetAttribute<StringValue>(attributeName));
      if (val != "")
      {
        rval[i] = val;
        i++;
  ...
  GetMatching(const char * attName, const char * attValue) const
  {
    ...
    for (it = this->begin(); it != this->end(); ++it)
    {
      ...
      if (current->GetAttribute<StringValue>(attName) == attValue)
      {
        return current;
      }
    }
