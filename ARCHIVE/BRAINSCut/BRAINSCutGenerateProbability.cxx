...
/** constructors */
BRAINSCutGenerateProbability ::BRAINSCutGenerateProbability( BRAINSCutDataHandler & dataHandler )
{
...
void
BRAINSCutGenerateProbability ::GenerateProbabilityMaps()
{
  ...
  for ( unsigned int currentROIAt = 0; currentROIAt < myDataHandler->GetROICount(); ++currentROIAt )
  {
          ...
          ++currentSubjectIt )
    {
      ...
      std::string currentROIFilename = ( *currentSubjectIt )->GetMaskFilenameByType( currentROIID );

      WorkingImageType::Pointer currentDeformedROI = ImageWarper< WorkingImageType >(
        currentRegistrationFilename, currentROIFilename, myDataHandler->GetAtlasImage() );
      /** add the deformed roi to accumulator */
      currentAccumulatedImages = AddImageToAccumulator( currentDeformedROI, currentAccumulatedImages );
    } /** end of iteration for subject */

    /** average the accumulator based on the counts */
    WorkingImagePointer currentProbabilityImage = ImageMultiplyConstant< WorkingImageType >(
      currentAccumulatedImages, 1.0F / static_cast< float >( currentROISubjectsCounter ) );

    /** get roi object */

    ProbabilityMapParser * currentROISet =
      myDataHandler->GetROIDataList()->GetMatching< ProbabilityMapParser >( "StructureID", currentROIID.c_str() );
    ...
    {
      std::cout << " Probability map directory does not exist. Create as following:: " << path.c_str() << std::endl;
...
void
BRAINSCutGenerateProbability ::GenerateSymmetricalSphericalCoordinateImage()
{
  const WorkingImageType::SizeType atlasImageSize(
    myDataHandler->GetAtlasImage()->GetLargestPossibleRegion().GetSize() );
  ...
  itk::ImageRegionIterator< WorkingImageType > it( myDataHandler->GetAtlasImage(),
                                                   myDataHandler->GetAtlasImage()->GetLargestPossibleRegion() );
  it.GoToBegin();
...
void
BRAINSCutGenerateProbability ::CreateNewFloatImageFromTemplate( WorkingImageType::Pointer &       PointerToOutputImage,
...
void
BRAINSCutGenerateProbability ::XYZToSpherical( const itk::Point< float, 3 > & LocationWithOriginAtCenterOfImage,
