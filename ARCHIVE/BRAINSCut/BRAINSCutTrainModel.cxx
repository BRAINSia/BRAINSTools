...
BRAINSCutTrainModel ::BRAINSCutTrainModel( BRAINSCutDataHandler & dataHandler )
...
BRAINSCutTrainModel ::~BRAINSCutTrainModel()
{
...
/** train */
void
BRAINSCutTrainModel ::InitializeTrainDataSet( bool doShuffle )
{
  ...
  {
    unsigned int numberOfSubSet = (float)this->m_trainingDataSet->GetTotalVectorSize() / (float)m_trainMaximumDataSize;
...
void
BRAINSCutTrainModel ::InitializeNeuralNetwork()
{
...
void
BRAINSCutTrainModel ::InitializeRandomForest()
{
...
void
BRAINSCutTrainModel ::TrainWithUpdate( cv::Ptr< OpenCVMLPType > myTrainer, pairedTrainingSetType & currentTrainData )
{
  // TODO change subset number properly
  myTrainer->setBackpropMomentumScale( 0.1 );
  myTrainer->setTrainMethod( OpenCVMLPType::RPROP );
  myTrainer->setTermCriteria(
    cvTermCriteria( CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, m_trainEpochIteration, m_trainDesiredError ) );
  myTrainer->train(
    cv::ml::TrainData::create( currentTrainData.pairedInput, cv::ml::ROW_SAMPLE, currentTrainData.pairedOutput ) );
}

void
BRAINSCutTrainModel ::SaveRFTrainModelAtIteration( cv::Ptr< cv::ml::RTrees > myTrainer, int depth, int NTrees )
{
  ...
  {
    std::cout << "Fail to save the model file ::" << std::endl << filename << std::endl;
    exit( EXIT_FAILURE );
  }
}

void
BRAINSCutTrainModel ::SaveANNTrainModelAtIteration( cv::Ptr< OpenCVMLPType > myTrainer, unsigned int No )
{
  char tempid[10];
  ...
  {
    std::cout << " A directory for training file does not exist. Create as following:: " << filename.c_str()
              << std::endl;
...
void
BRAINSCutTrainModel ::recordRFTrainInformation( int depth, int nTree )
{
  char cDepth[10];
...
void
BRAINSCutTrainModel ::appendToFile( std::string filename, std::string line )
{
  std::fstream filestr;
  ...
  if ( !filestr.good() )
  {
    std::cout << "Cannot open the file :: " << filename << std::endl << "Fail to append line  :: " << line << std::endl;
    exit( EXIT_FAILURE );
  }
...
void
BRAINSCutTrainModel ::printANNTrainInformation( cv::Ptr< OpenCVMLPType > /*NOT USED myTrainer */, unsigned int No )
{
  std::cout << " Error, "
            << " NOT_COMPUTED " /* myTrainer.get_MSE() This needs to be instumented again */
            << " Iteration, " << No << std::endl;
}

void
BRAINSCutTrainModel ::FillANNLayerStructureArray3D( int * const layer ) const
{
...
BRAINSCutTrainModel ::TrainANN()
{
  ...
  std::cout << "ANNLayer::" << annLayer << std::endl;
  trainner->setLayerSizes( annLayer );
  std::cout << "getLayerSizes::" << trainner->getLayerSizes() << std::endl;
  for ( unsigned int currentIteration = 1; currentIteration <= m_trainIteration; currentIteration++ )
  {
    unsigned int subSetNo = ( currentIteration - 1 ) % this->m_trainingDataSet->GetNumberOfSubSet();
    TrainWithUpdate( trainner, *( this->m_trainingDataSet->GetTrainingSubSet( subSetNo ) ) );
...
void
BRAINSCutTrainModel ::TrainRandomForest()
{
...
void
BRAINSCutTrainModel ::TrainRandomForestAt( const int depth, const int numberOfTree )
{
  ...
  forest->setTermCriteria( cv::TermCriteria( cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, numberOfTree, 0.1F ) );
...
void
BRAINSCutTrainModel ::SetRFErrorFilename()
{
  const std::string basename = m_myDataHandler.GetModelBaseName();

  if ( basename.empty() )
  {
    std::cout << "Model Basename has to be set first." << std::endl << __LINE__ << "::" << __FILE__ << std::endl;
    exit( EXIT_FAILURE );
  }
...
void
BRAINSCutTrainModel ::SetRFErrorFile()
{
  std::fstream  filestr;
  ...
  if ( ifile )
  {
    std::cout << "file already exist. Append to the file" << std::endl;
    ifile.close();
    return;
  }
