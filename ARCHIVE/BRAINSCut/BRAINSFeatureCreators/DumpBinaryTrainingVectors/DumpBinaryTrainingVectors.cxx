...
void
ReadHeader( const char * fname, unsigned int & InputVectorSize, unsigned int & OutputVectorSize,
...
main( int argc, char ** argv )
{
  ...
  if ( inputVectorFilename.empty() || inputHeaderFilename.empty() )
  {
  ...
  try
  {
    ReadHeader( inputHeaderFilename.c_str(), InputVectorSize, OutputVectorSize, NumberTrainingVectorsFromFile );
    std::cout << "Input Vector  Size " << InputVectorSize << "Output Vector Size " << OutputVectorSize
              << " #Training Vectors " << NumberTrainingVectorsFromFile << std::endl;
  }
  ...
  unsigned int recordsize = ( InputVectorSize + OutputVectorSize + SentinalValueSize ) * sizeof( float );
  float *      buf = new float[InputVectorSize + OutputVectorSize + SentinalValueSize];
  ...
  for ( unsigned int i = 0; i < NumberTrainingVectorsFromFile; i++ )
  {
    if ( binfile.eof() )
    {
      std::cerr << "Premature end of file at record " << i << std::endl;
      result = 1;
      break;
    }
    binfile.read( (char *)buf, recordsize );
    for ( unsigned int j = 0; j <= OutputVectorSize + InputVectorSize; j++ )
    {
      std::cout << buf[j] << " ";
    }
    std::cout << std::endl;
    if ( buf[OutputVectorSize + InputVectorSize + SentinalValueSize - 1] != LineGuard )
    {
      std::cerr << "Record not properly terminated by sentinel value :::: "
                << buf[OutputVectorSize + InputVectorSize + SentinalValueSize - 1] << " != " << LineGuard << std::endl;
      result = 1;
      break;
    }
  }
