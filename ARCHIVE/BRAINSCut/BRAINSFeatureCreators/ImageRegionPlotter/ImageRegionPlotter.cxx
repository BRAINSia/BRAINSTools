...
main(int argc, char * argv[])
{
  ...
  if ((!inputLabelVolume.empty()) && (!inputVolume1.empty()) && (!inputVolume2.empty()))
  {
    ...
    outputFileStream << "[Image1]: " << inputVolume1 << std::endl
                     ...
                     << "[LabelMap]: " << inputLabelVolume << std::endl;
    ...
    for (LabelMapType::iterator iter = labelIndex.begin(); iter != labelIndex.end(); ++iter)
    {
      ...
      for (int i = 0; i < 101; i++)
      {
        for (int j = 0; j < 101; j++)
        {
          ...
          outputFileStream << iter->first << "," << i << "," << j << "," << HistogramArray[iter->second][i][j]
                           << std::endl;
        }
      }
    }
  ...
  else
  {
    std::cout << " Wrong Argument! " << std::endl
              << " inputVolume1, inputVolume2, and inputLabelVolume are necessary! " << std::endl;
    return EXIT_FAILURE;
  }
  return 0;
}
