...
main( int argc, char * argv[] )
{
  ...
  for ( it.GoToBegin(); !it.IsAtEnd(); ++it )
  {
    ...
    {
      if ( currentIndexOfY[dimIndex] >
           static_cast< InputImageType::IndexType::IndexValueType >( imageInYSize[dimIndex] ) )
      {
    ...
    if ( imageInBoundary )
    {
      ...
      if ( inputMaskVolumeInXAxis.empty() ||
           ( ( !inputMaskVolumeInXAxis.empty() ) && ( !inputMaskVolumeInYAxis.empty() ) &&
             ( binaryImageInX->GetPixel( currentIndexOfX ) > 0 ) &&
             ( binaryImageInY->GetPixel( currentIndexOfY ) > 0 ) ) )
      {
