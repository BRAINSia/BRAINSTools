...
def GetProbabilityFilename(inputROI, inputDir):

    outputFilename = (
        inputDir + "/" + "/probabilityMaps/" + inputROI + "_ProbabilityMap.nii.gz"
    )
...
def ThresholdProbabilityMap(
    inputVolume, lowerThreshold, upperThreshold, outputFilename
):
    import os
    import sys
    ...
    outImg = sitk.BinaryThreshold(inImg, lowerThreshold, upperThreshold)
...
def SmoothProbabilityMap(inputVolume, inputSigma, outputVolume):
    import os
    import sys
    import SimpleITK as sitk
    ...
    if inputSigma <= 0:
        ...
        outImg = smoother.Execute(inImg, inputSigma, normalizeAcrossScale)
...
def NormalizeInputVolume(inputVolume, inputMethod, inputStats, outputVolume):
    import SimpleITK as sitk
    import os
    from math import e
    ...
    if inputMethod == "zScore":
        ...
        outImg = old_div(
            (inImg - inputStats["Minimum"]),
            (inputStats["Maximum"] - inputStats["Minimum"]),
        )
    elif inputMethod == "DoubleSigmoid":
        print("Double Sigmoid Normalization")
        outMsk1 = sitk.BinaryThreshold(
            inImg, inputStats["Minimum"], inputStats["Median"]
        )
        outImg1 = old_div(
            1,
            (
                ...
                ** (
                    -2
                    * (inImg - (inputStats["Median"]))
                    / (inputStats["Median"] - inputStats["Quantile25"])
                )
            ),
        )

        outMsk2 = sitk.BinaryThreshold(
            inImg, (inputStats["Median"] + 0.00001), inputStats["Maximum"]
        )
        outImg2 = old_div(
            1,
            (
        ...
        outImg = outImg1 * sitk.Cast(outMsk1, sitk.sitkFloat32) + outImg2 * sitk.Cast(
            outMsk2, sitk.sitkFloat32
        )
...
def NormalizeAndComputeStatOfROI(
    inputSet_LabelStat, inputMethod, outputVolume, outputCSVFilename
):
    from MyUtilities import NormalizeInputVolume

    m_outputVolume = NormalizeInputVolume(
        inputSet_LabelStat["volume"],
        inputMethod,
    ...
    m_outputCSV, m_outputDictSet = LabelStatistics(
        inputSet_LabelStat["labelVolume"], m_outputVolume, outputCSVFilename
    )
