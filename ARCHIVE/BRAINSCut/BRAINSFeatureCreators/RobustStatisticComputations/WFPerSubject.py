## start {
def WFPerSubjectDef(
    inputListOfSubjectVolumes, inputTemplateDir, outputCacheDir, outputResultsDir
):
    ...
    BFitAtlasToSubject = pe.Node(
        interface=BRAINSFit(), name="01_AtlasToSubjectRegistration"
    )
    ...
    BFitAtlasToSubject.inputs.transformType = (
        "ScaleVersor3D,ScaleSkewVersor3D,Affine,BSpline"
    )
    ...
    BFitAtlasToSubject.inputs.costFunctionConvergenceFactor = 1e09
    ...
    WFPerSubject.connect(
        BFitAtlasToSubject, "outputTransform", datasink, "01_BFitAtlasToSubject"
    )
    ...
    GetProbabilityMapFilename = pe.Node(
        name="utilGetProbabilityMapFilename",
        interface=Function(
            input_names=["inputROI", "inputDir"],
    ...
    WarpProbabilityMap = pe.Node(
        interface=BRAINSResample(), name="02_WarpProabilityMap"
    )
    ...
    WFPerSubject.connect(
        GetProbabilityMapFilename, "outputFilename", WarpProbabilityMap, "inputVolume"
    )
    WFPerSubject.connect(
        BFitAtlasToSubject, "outputTransform", WarpProbabilityMap, "warpTransform"
    )
    WFPerSubject.connect(
        WarpProbabilityMap, "outputVolume", datasink, "02_WarpedProbability"
    )
    ...
    sigmaInputNode = pe.Node(
        interface=IdentityInterface(fields=["sigma"]), name="sigmaInput"
    )
    sigmaInputNode.iterables = ("sigma", [0, 0.5, 1, 1.5, 2])
    WFPerSubject.add_nodes([sigmaInputNode])

    SmoothingProbMap = pe.Node(
        name="SmoothingProbMap",
        interface=Function(
            input_names=["inputVolume", "inputSigma", "outputVolume"],
            output_names=["outputSmoothProbabilityMap"],
            function=MyUtilities.SmoothProbabilityMap,
        ),
    )
    WFPerSubject.add_nodes([SmoothingProbMap])
    WFPerSubject.connect(sigmaInputNode, "sigma", SmoothingProbMap, "inputSigma")
    WFPerSubject.connect(
        WarpProbabilityMap, "outputVolume", SmoothingProbMap, "inputVolume"
    )
    ...
    WFPerSubject.connect(
        SmoothingProbMap, "outputSmoothProbabilityMap", CreateMask, "inputVolume"
    )

    WFPerSubject.connect(CreateMask, "outputMask", datasink, "03_ROI")
    #
    # --------------------------------------------------------------------------------------- #
    # 4. Compute Statistice
    #
    LabelStatistics = pe.Node(
        name="04_LabelStatistics",
        interface=Function(
            input_names=["inputLabel", "inputVolume", "outputCSVFilename"],
            output_names=["outputCSVFilename", "outputDictionarySet"],
            function=MyUtilities.LabelStatistics,
        ),
    )
    LabelStatistics.iterables = (
        "inputVolume",
        [inputListOfSubjectVolumes["t1"], inputListOfSubjectVolumes["t2"]],
    )
    LabelStatistics.inputs.outputCSVFilename = "labelStatistics.csv"

    WFPerSubject.add_nodes([LabelStatistics])
    WFPerSubject.connect(CreateMask, "outputMask", LabelStatistics, "inputLabel")
    WFPerSubject.connect(
        LabelStatistics, "outputCSVFilename", datasink, "labelStatistics"
    )
    # --------------------------------------------------------------------------------------- #
    ...
    normalizationListNode = pe.Node(
        interface=IdentityInterface(fields=["normalization"]), name="normalization"
    )
    ...
    NormalizeAndGetStatofROI_Node = pe.Node(
        name="05_NormalizeStats",
        interface=Function(
            input_names=[
                "inputSet_LabelStat",
                "inputMethod",
            ...
            output_names=["outputCSV", "outputVolume"],
    ...
    WFPerSubject.connect(
        LabelStatistics,
        "outputDictionarySet",
        NormalizeAndGetStatofROI_Node,
        "inputSet_LabelStat",
    )
    WFPerSubject.connect(
        normalizationListNode,
        "normalization",
        NormalizeAndGetStatofROI_Node,
        "inputMethod",
    )
    ...
    # datasink
    WFPerSubject.connect(
        NormalizeAndGetStatofROI_Node, "outputCSV", datasink, "labelStatistics.@Stat"
    )
