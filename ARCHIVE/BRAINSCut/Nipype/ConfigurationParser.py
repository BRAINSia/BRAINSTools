...
def ConfigurationSectionMap(configurationFilename):
    ...
    if os.path.exists(configurationFilename):
        ...
        print(
            (
                """ERROR in ConfigurationSectionMap
        ...
        for option in m_configuration.options(section):
            try:
                if option in dictionaryListSet:
                    sectionDict[option] = ast.literal_eval(
                        m_configuration.get(section, option)
                    )
                else:
                ...
                print(
                    (
                        "{option} ==>  {value}".format(
                            option=option, value=sectionDict[option]
                        )
                    )
                )
            except:
...
def BRAINSCutCMDFromConfigFile(
    configurationFilename,
    xmlFilename,
    probabilityMapDict,
    ...
    for sessionID in list(applyModelOutputDirDict.keys()):
        p_applyModelOutputDirDict[sessionID] = os.path.abspath(
    ...
    for option in list(methodParameter.keys()):
        optionStr = optionStr + " {option} {value} ".format(
            option=option, value=methodParameter[option]
        )
...
def BRAINSCutGenerateProbabilityMap(
    configurationFilename, probabilityMapDict, gaussianSigma, outputXmlFilename
):
    print(
        """*****************************
...
def updating(originalFilename, editedFilename, whatToChangeDict):
    import configparser
            ...
            if option in whatToChangeDict:
                print(
                    (
                      ...
                      """.format(
                            op=option, value=whatToChangeDict[option]
                        )
                    )
                )
                ...
                outConfigParser.set(
                    section, option, inConfigParser.get(section, option)
                )
...
def ConfigurationFileEditor(originalFilename, editedFilenamePrefix):
    ...
    for roi in list(roiDict.keys()):
        ...
        editedFilenames[roi] = updating(
            originalFilename, new_ConfigFilename, whatToChange
        )
