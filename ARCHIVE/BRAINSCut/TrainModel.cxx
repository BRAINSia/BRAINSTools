                              ...
                              int       verbose = 0)
{
  ...
  {
    itkGenericExceptionMacro(<< " Current Subset number ," << SubSetNumber << " can not exceed total number of subset, "
                             << NumSubSets);
  }
  int                  NumInputVectors;
  ...
  int                 verbose)
{
  ...
  std::cout << "\nEstimated Memory Needed for Vectors: (MB) "
            << ((static_cast<float>(MaximumVectorsPerEpoch)) * sizeof(neural_scalar_type) * (InputVectorSize + 1.0F)) /
                 (1024.0F * 1024.0F)
            << std::endl;
           ...
           (SubSetNumber = ((SubSetNumber == NumSubSets - 1) ? 0 : SubSetNumber + 1)))
  {
    ...
    if (!itksys::SystemTools::FileExists(temp_Weights.c_str()))
    {
      ...
      t_diff = (int)(1000.0 * (t_current.tv_sec - t_start.tv_sec) + (t_current.tv_usec - t_start.tv_usec) / 1000.0);
    }
    else
    {
