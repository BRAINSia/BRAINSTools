...
public:
  ...
  std::string
  Get( const char * eleType, const char * key )
  {
  ...
  std::string
  GetIfExist( const char * eleType, const char * key )
  {
    StringMap::const_iterator it = this->find( std::string( key ) );
    ...
    if ( it == this->end() )
    {
      std::cout << " *** Note ***" << std::endl
                << " *** Note ***" << std::endl
                << " Missing Name of attribute, mark as NA " << eleType << " " << key << std::endl
                << std::endl;
      return "NA";
    }
    return it->second;
...
{
public:
  XMLConfigurationFileParser( const std::string & filename )
    : XMLParser( filename )
  {
  ...
  void
  StartElement( void * userData, const XML_Char * name, const XML_Char ** atts ) override;

  void
  EndElement( void * userData, const XML_Char * name ) override;
