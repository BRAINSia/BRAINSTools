...
void
cnpy::parse_npy_header( FILE * fp, unsigned int & word_size, unsigned int *& shape, unsigned int & ndims,
                        bool & fortran_order )
{
  char   buffer[256];
...
getUshortFromChar( const char * const in )
{
  unsigned short out;
...
static unsigned int
getUintFromChar( const char * const in )
{
  unsigned int out;
  char * const ptr = reinterpret_cast< char * >( &out );
  ptr[0] = in[0];
...
void
cnpy::parse_zip_footer( FILE * fp, unsigned short & nrecs, unsigned int & global_header_size,
