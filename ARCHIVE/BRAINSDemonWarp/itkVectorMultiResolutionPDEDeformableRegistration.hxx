...
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::VectorMultiResolutionPDEDeformableRegistration()
{
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::SetMovingImage( const MovingImageType * ptr )
{
 ...
 */
template < typename TFixedImage, typename TMovingImage, typename TDisplacementField, typename TRealType >
const typename VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                               TRealType >::MovingImageType *
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::GetMovingImage( void ) const
{
  return dynamic_cast< const MovingImageType * >( this->ProcessObject::GetInput( 1 ) );
}
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::SetFixedImage( const FixedImageType * ptr )
{
...
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::GetFixedImage( void ) const
{
  return dynamic_cast< const FixedImageType * >( this->ProcessObject::GetInput( 0 ) );
}

template < typename TFixedImage, typename TMovingImage, typename TDisplacementField, typename TRealType >
std::vector< SmartPointer< DataObject > >::size_type
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::GetNumberOfValidRequiredInputs() const
{
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::SetNumberOfLevels( unsigned int num )
{
...
template < typename TFixedImage, typename TMovingImage, typename TDisplacementField, typename TRealType >
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField, TRealType >::PrintSelf(
                                                ...
                                                TRealType >::GenerateData()
{
  ...
  DisplacementFieldPointer inputPtr = const_cast< DisplacementFieldType * >( this->GetInput( 2 ) );
  ...
  else if ( inputPtr )
  {
    ...
    for ( unsigned int dim = 0; dim < DisplacementFieldType::ImageDimension; ++dim )
    {
      ...
      sigma *= fixedVectorImage->GetSpacing()[dim] / inputPtr->GetSpacing()[dim];
  ...
  while ( !this->Halt() )
  {
    ...
    typename ImageToVectorImageType::Pointer vectorFixedImage = ImageToVectorImageType::New();
    typename ImageToVectorImageType::Pointer vectorMovingImage = ImageToVectorImageType::New();
    for ( unsigned int i = 0; i < this->GetFixedImage()->GetVectorLength(); ++i )
    {
      vectorFixedImage->SetInput( i, m_FixedVectorImagePyramid[i]->GetOutput( fixedLevel ) );
      vectorMovingImage->SetInput( i, m_MovingVectorImagePyramid[i]->GetOutput( movingLevel ) );
    }
    try
    {
  ...
  if ( !lastShrinkFactorsAllOnes )
  {
    ...
    m_FieldExpander->SetInput( tempField );
    m_FieldExpander->SetSize( fixedVectorImage->GetLargestPossibleRegion().GetSize() );
    m_FieldExpander->SetOutputStartIndex( fixedVectorImage->GetLargestPossibleRegion().GetIndex() );
...
template < typename TFixedImage, typename TMovingImage, typename TDisplacementField, typename TRealType >
bool
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField, TRealType >::Halt()
{
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::GenerateOutputInformation()
{
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::GenerateInputRequestedRegion()
{
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::EnlargeOutputRequestedRegion( DataObject * ptr )
{
  // call the superclass's implementation
...
void
VectorMultiResolutionPDEDeformableRegistration< TFixedImage, TMovingImage, TDisplacementField,
                                                TRealType >::VerifyInputInformation() const
{
