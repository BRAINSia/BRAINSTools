...
main( int argc, char ** argv )
{
  ...
  std::string extension = itksys::SystemTools::LowerCase( itksys::SystemTools::GetFilenameExtension( inputSurface ) );
  ...
  vtkFloatArray * curvature =
    vtkFloatArray::SafeDownCast( surfaceData->GetPointData()->GetAbstractArray( "Mean_Curvature" ) );
  ...
  if ( testDepth )
  {
    for ( unsigned int i = 1; i < ( arraySize + 1 ); i++ )
    {
      ...
      if ( ( ( fabs( gyralDepthResults[i - 1] ) - fabs( gyralThickness->GetValue( i ) ) ) /
             fabs( gyralThickness->GetValue( i ) ) ) >= 0.001 )
      {
  ...
  if ( testArea )
  {
    for ( unsigned int i = 1; i < ( arraySize + 1 ); i++ )
    {
      if ( ( ( fabs( totalAreaResults[i - 1] ) - fabs( surfaceArea->GetValue( i ) ) ) /
             fabs( surfaceArea->GetValue( i ) ) ) >= 0.001 )
      {
        std::cerr << "Error: Expected total area (" << surfaceArea->GetValue( i ) << ") measured (";
        std::cerr << totalDepthResults[i - 1] << " )" << std::endl;
        result = EXIT_FAILURE;
      }
      if ( ( ( fabs( gyralAreaResults[i - 1] ) - fabs( gyralArea->GetValue( i ) ) ) /
             fabs( gyralArea->GetValue( i ) ) ) >= 0.001 )
      {
        std::cerr << "Error: Expected gyral area (" << gyralArea->GetValue( i ) << ") measured (";
        std::cerr << gyralDepthResults[i - 1] << " )" << std::endl;
        result = EXIT_FAILURE;
      }
      if ( ( ( fabs( sulcalAreaResults[i - 1] ) - fabs( sulcalArea->GetValue( i ) ) ) /
             fabs( sulcalArea->GetValue( i ) ) ) >= 0.001 )
      {
