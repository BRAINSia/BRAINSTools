...
/*=========================================================================
  ...
  Module:    $RCSfile: vtkPolyDataConnectivityIDFilter.cxx,v $
...
vtkPolyDataConnectivityIDFilter::vtkPolyDataConnectivityIDFilter()
{
...
int
vtkPolyDataConnectivityIDFilter::RequestData(vtkInformation *        vtkNotUsed(request),
                                             ...
                                             vtkInformationVector *  outputVector)
{
    ...
    for (cellId = 0; cellId < numCells; cellId++)
    {
      ...
      if (this->Visited[cellId] < 0)
      {
        ...
        if (this->NumCellsInRegion > maxCellsInRegion)
        {
          ...
          this->LargestRegionId = this->RegionNumber;
        }
  ...
  if (this->ExtractionMode == VTK_EXTRACT_POINT_SEEDED_REGIONS ||
      this->ExtractionMode == VTK_EXTRACT_CELL_SEEDED_REGIONS ||
      this->ExtractionMode == VTK_EXTRACT_CLOSEST_POINT_REGION || this->ExtractionMode == VTK_EXTRACT_ALL_REGIONS)
  ...
  else // extract largest region
  {
    for (cellId = 0; cellId < numCells; cellId++)
    {
      if (this->Visited[cellId] == this->LargestRegionId)
      {
        this->Mesh->GetCellPoints(cellId, npts, pts);
        this->PointIds->Reset();
        for (i = 0; i < npts; i++)
...
//
void
vtkPolyDataConnectivityIDFilter::TraverseAndMark()
{
      ...
      if (this->Visited[cellId] < 0)
      {
        ...
        for (j = 0; j < npts; j++)
        {
          if (this->PointMap[ptId = pts[j]] < 0)
          {
            ...
            vtkIdTypeArray::SafeDownCast(this->NewScalars)->SetValue(this->PointMap[ptId], this->RegionNumber);
          }
...
int
vtkPolyDataConnectivityIDFilter::GetNumberOfExtractedRegions()
{
  return this->RegionSizes->GetMaxId() + 1;
}

// Obtain the largest region id
int
vtkPolyDataConnectivityIDFilter::GetLargestRegionId()
{
  return this->LargestRegionId;
}

// Initialize list of point ids/cell ids used to seed regions.
void
vtkPolyDataConnectivityIDFilter::InitializeSeedList()
{
  this->Modified();
  this->Seeds->Reset();
}
...
void
vtkPolyDataConnectivityIDFilter::AddSeed(int id)
{
  this->Modified();
...
void
vtkPolyDataConnectivityIDFilter::DeleteSeed(int id)
{
  this->Modified();
...
void
vtkPolyDataConnectivityIDFilter::InitializeSpecifiedRegionList()
{
  this->Modified();
...
void
vtkPolyDataConnectivityIDFilter::AddSpecifiedRegion(int id)
{
  this->Modified();
...
void
vtkPolyDataConnectivityIDFilter::DeleteSpecifiedRegion(int id)
{
  this->Modified();
...
void
vtkPolyDataConnectivityIDFilter::PrintSelf(ostream & os, vtkIndent indent)
{
