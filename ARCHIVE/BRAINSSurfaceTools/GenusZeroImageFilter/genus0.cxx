...
static int *  persist = nullptr;

static void **         calloc_list = nullptr;
...
{
  fputs( msg, stdout ); //  fprintf(stdout, msg);
}

static void *
basic_calloc( size_t nelem, size_t elsize )
{
...
static void
Gfree_all( int keep_persist )
{
  int i;

  if ( calloc_list == nullptr )
  {
...
static void
error_msg( const char * msg, int line )
{
  char line_msg[100];
...
static void *
Gcalloc( size_t nelem, size_t elsize, int make_persist )
{
  void ** cl;
  int *   p, j;
  ...
  if ( calloc_list[calloced] == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return nullptr;
  }
...
static void
process_row( int axis, float * start )
{
  register size_t len;
...
static void
recursive_add_dist_squared( int axis, float * start )
{
  size_t len;
...
static int
dist_squared( int rank, size_t * axis_len, float * deltax, register char * inimage, register char inobject,
              float * outdist_squared )
{
  ...
  for ( i = 0; i < rank; i++ )
  {
    ...
    if ( max_axis_len < axis_len[i] )
    {
      max_axis_len = axis_len[i];
    }
    g_stride[i] = data_len;
    data_len *= axis_len[i];
  }
...
static int
sub2ind( int horiz, int vert, int depth, int _img_horiz, int _img_vert )
{                                                           /* convert
                                                                subscripts to
                                                                linear index */
  return horiz + ( vert + depth * _img_vert ) * _img_horiz; /* factoring to
                                                                save a mult...
                                                                why not? */
}

// static int get_cc(unsigned char * zpic, int *que, int * status, int *dims,
// int *ac, int connectivity)
static int
get_cc( unsigned char * _zpic, int * _que, int * _status, size_t * dims, int * ac, int _connectivity )
{
  int i, *_offs, nbrs6[6], _nbrs18[18], *g_counts;
  int j, k, h, vox, c, w, groups, _que_pos, _que_len;
  int _autocrop[3][2], woffsh, w0;
  ...
  for ( k = h = 0; k < 3; k++ )
  {
    for ( j = 0; j < 3; j++ )
    {
      for ( i = 0; i < 3; i++ )
      {
        if ( !( ( i == 0 || i == 2 ) && ( j == 0 || j == 2 ) && ( k == 0 || k == 2 ) ) )
        {
          if ( ( i != 1 ) || ( j != 1 ) || ( k != 1 ) ) /* calculate offsets
                                                          for all 18
                                                          neighboring voxels */
          {
            _nbrs18[h++] = sub2ind( i, j, k, dims[0], dims[1] ) - sub2ind( 1, 1, 1, dims[0], dims[1] );
          }
        }
      }
    }
  ...
  for ( k = 0; k < (int)dims[2]; k++ )
  {
    for ( j = 0; j < (int)dims[1]; j++ )
    {
      ...
      {
        if ( _zpic[w] == 1 ) /* not processed yet */
        {
          groups++; /* new group */
          _status[w] = groups;
          // std::cout << "Set Status " << w << " " << groups << std::endl;
          _zpic[w] = 2; /* 0=background, 1=not on que, 2=on que or done with */
          _que_len = 0;
          for ( h = 0; h < c; h++ ) /* add nbrs to que */
          {
            if ( _zpic[woffsh = w + _offs[h]] == 1 )
            {
              _zpic[que[_que_len++] = woffsh] = 2;
            }
          }

          _que_pos = 0;
          while ( _que_pos < _que_len )
          {
            _status[w0 = _que[_que_pos++]] = groups; /* get a voxel from que */
            // std::cout << "Status que " << w0 << " " << _que_pos << " " <<
            // que[_que_pos] << " " << groups << std::endl;
            for ( h = 0; h < c; h++ ) /* add nbrs to que */
            {
              if ( _zpic[woffsh = w0 + _offs[h]] == 1 )
              {
                _zpic[_que[_que_len++] = woffsh] = 2;
              }
            }
          }
        }
  ...
  for ( k = 0; k < 3; k++ )
  {
    _autocrop[k][0] = dims[k];
    _autocrop[k][1] = 0;
  }
  for ( k = h = 0; k < (int)dims[2]; k++ )
  {
    for ( j = 0; j < (int)dims[1]; j++ )
    {
      for ( i = 0; i < (int)dims[0]; i++ )
...
set_up( genus0parameters * g0 )
{
  ...
  if ( ( g0->connectivity != 6 ) && ( g0->connectivity != 18 ) )
  {
    g0->connectivity = 6;
  }
  ...
  paddeddims[0] = img_horiz;
  paddeddims[1] = img_vert;
  paddeddims[2] = img_depth;

  if ( ( zpic = (unsigned char *)Gcalloc( totlen, sizeof( unsigned char ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  ...
  {
    que_size = totlen;
    if ( ( que = (int *)Gcalloc( que_size, sizeof( int ), 0 ) ) == nullptr )
    {
      error_msg( "Memory error.\n", __LINE__ );
      return 1;
    }
    ...
    if ( ( status = (int *)Gcalloc( totlen, sizeof( int ), 0 ) ) == nullptr )
    {
      error_msg( "Memory error.\n", __LINE__ );
      return 1;
    }
  ...
  for ( i = 0; i < 3; i++ )
  {
    if ( ( g0->extraijkscale )[i] <= 0.0 )
    {
      ( g0->extraijkscale )[i] = 1.0;
    }
  }
  for ( i = 0; i < 3; i++ )
  {
    voxelsize[i] *= ( g0->extraijkscale )[i];
  }
  minvoxelsize = voxelsize[0];
  if ( minvoxelsize > voxelsize[1] )
  {
    minvoxelsize = voxelsize[1];
  }
  if ( minvoxelsize > voxelsize[2] )
  {
    minvoxelsize = voxelsize[2];
  }
  if ( minvoxelsize <= 0.0 )
  {
    minvoxelsize = 1.0;
  ...
  nbrs[2] = -img_horiz;
  nbrs[3] = -nbrs[2];
  nbrs[4] = -( img_horiz * img_vert );
  nbrs[5] = -nbrs[4];
  for ( k = h = 0; k < 3; k++ )
  {
    for ( j = 0; j < 3; j++ )
    {
      for ( i = 0; i < 3; i++ ) /* calculate offsets for all 27 voxels in
                                  neighborhood */
      {
  ...
  for ( k = h = 0; k < 3; k++ )
  {
    for ( j = 0; j < 3; j++ )
    {
      for ( i = 0; i < 3; i++ )
      {
        if ( ( i != 1 ) || ( j != 1 ) || ( k != 1 ) ) /* calculate offsets for
  ...
  {
    que_size = totlen;
    if ( que == nullptr ) /* might have already calloced it above ... */
    {
      if ( ( que = (int *)Gcalloc( que_size, sizeof( int ), 0 ) ) == nullptr )
      {
        error_msg( "Memory error.\n", __LINE__ );
    ...
    if ( ( fzpic = (float *)Gcalloc( totlen, sizeof( float ), 0 ) ) == nullptr )
    {
      error_msg( "Memory error.\n", __LINE__ );
      return 1;
    }
    ...
    if ( ( cm = (int *)Gcalloc( cm_size, sizeof( int ), 0 ) ) == nullptr )
    {
      error_msg( "Memory error.\n", __LINE__ );
      return 1;
    }
    ...
    /* initialize voxel status */
    if ( status == nullptr ) /* might have already calloced it above ... */
    {
      if ( ( status = (int *)Gcalloc( totlen, sizeof( int ), 0 ) ) == nullptr )
      {
        error_msg( "Memory error.\n", __LINE__ );
    ...
    for ( i = 0; i < totlen; i++ )
    {
      status[i] = ( zpic[i] == g0->cut_loops ) ? 0 : 3; /* 3 if not wanted
                                                          component */
    }
...
static int
truecm( int st ) /* return true component, set cm[st] to true comp */
{
  int s0, s1;

  // std::cout << std::endl << "In True CM " << st << " " << cm[st] <<
  // std::endl;
  if ( cm[st] != st )
  {
    s0 = st;
    ...
    {
      st = cm[st];
      // std::cout << "While Loop 1 " << st << std::endl;
    }
...
static int
truecmvx( int vx ) /* return true component of voxel */
{
  return status[vx] = truecm( status[vx] );
}

static int
test18( int qqp, int * nc )
{
  ...
  for ( i = 0; i < 18; i++ ) /* for each 18 neighbor */
  {
    if ( status[qqp + nbrs18[i]] > 10 ) /* if nbr is in an Mcubes component */
    {
      if ( !ec[idx[i]] ) /* if not assigned an edge component */
      {
        ...
        st[++ec_count] = stqqpn; /* remember the status associated with the new
                                   component */
  ...
  *nc = ( ec_count > 0 ) ? ( st[1] ) : 0;
  // std::cout << "TEST18 : " << found_another << " " << *nc << std::endl;
  return found_another;
}

static int
test6( int qqp, int * nc )
{
  int        elQqpj, stqqpn, i, j, ec_count = 0, ec[27], found_another = 1;
  ...
  pass[1] = pass[3] = pass[5] = pass[7] = pass[9] = pass[11] = pass[15] = pass[17] = pass[19] = pass[21] = pass[23] =
    pass[25] = 0;
  if ( status[qqp + offs[4]] > 10 )
  {
    pass[1] = pass[3] = pass[5] = pass[7] = 1;
  }
  if ( status[qqp + offs[10]] > 10 )
  {
    pass[1] = pass[9] = pass[11] = pass[19] = 1;
  }
  if ( status[qqp + offs[12]] > 10 )
  {
    pass[3] = pass[9] = pass[15] = pass[21] = 1;
  }
  if ( status[qqp + offs[14]] > 10 )
  {
    pass[5] = pass[11] = pass[17] = pass[23] = 1;
  }
  if ( status[qqp + offs[16]] > 10 )
  {
    pass[7] = pass[15] = pass[17] = pass[25] = 1;
  }
  if ( status[qqp + offs[22]] > 10 )
  {
    pass[19] = pass[21] = pass[23] = pass[25] = 1;
  }
...
cmtostat( void )
{
  int  j, i, *cmremap = nullptr, *ccount = nullptr, totlen;
  char msg[200];

  // std::cout << "COUNT SIZE " << comp_count << std::endl;
  if ( ( cmremap = (int *)Gcalloc( comp_count + 1, sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  if ( ( ccount = (int *)Gcalloc( comp_count + 1, sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  // std::cout << "Input comp_count " << comp_count << std::endl;
  for ( i = 11; i <= comp_count; i++ )
  {
    ccount[truecm( i )]++;
    // std::cout << i << " TRUE CM " << truecm(i);
    // std::cout << " CCOUNT " << ccount[truecm(i)] << std::endl;
  }
  j = 10;
  for ( i = 11; i <= comp_count; i++ )
  {
    if ( ccount[i] )
    {
      ...
      // std::cout << "I " << i;
      // std::cout << " J " << j;
      // std::cout << " ccount " << ccount[i];
      // std::cout << " cmremap " << cmremap[i] << std::endl;
    }
  }
  totlen = img_horiz * img_vert * img_depth;
  for ( i = 0; i < totlen; i++ )
  {
    if ( status[i] > 10 )
    {
      // std::cout << "Set Status I " << i << " " << status[i] << " " <<
      // cm[status[i]] << " " << cmremap[cm[status[i]]] << std::endl;
      status[i] = cmremap[cm[status[i]]];
    }
  }
  if ( verbose )
  {
  ...
  comp_count = j;
  // std::cout << "Set comp_count " << comp_count << std::endl;
  for ( i = 11; i <= comp_count; i++ )
  {
    cm[i] = i;
  }
...
find_component( int level )
{
  ...
  // std::cout << "find_component: " << level << std::endl;
  if ( cut_loops )
  {
    theconnectivity = connectivity;
  }
  else
  {
    theconnectivity = invconnectivity;
  }
  ...
  // std::cout << "theconnectivity: " << theconnectivity << std::endl;

  if ( theconnectivity == 18 )
  {
    nbrs0 = nbrs18;
    test = test18;
  }
      ...
      while ( que_pos != que_len )
      {
        ...
        if ( found_another )
        {
          if ( nc == 0 ) /* if no neighboring component */
          {
            ...
            // std::cout << "QQP Status " << qqp << " " << comp_count <<
            // std::endl;
          }
          else
          {
            ...
            for ( i = 0; i < theconnectivity; i++ )
            {
              ...
              if ( status[qqpni] > 10 ) /* if part of a component */
              {
                                                ...
                                                */
                {
                  cm[status[qqpni]] = nc;
                  status[qqpni] = nc;
                  // std::cout << "QQPNI Status " << qqpni << " " <<
                  // status[qqpni] << std::endl;
                }
              }
            }
          }
...
static int
GetSurf( unsigned char * J, unsigned char val, int * dims, int _connectivity, int ** Tris, float ** Verts,
         int * Tri_count, int * Vert_count, genus0parameters * g0 )
{
  unsigned char *_status, *cidx, *pidx;
  int            _img_vert, iv1, _img_horiz, _img_depth, cellplane, cellplanem[3];
  int            i, j, k, u, v, w, *v_idx, tri_count, vert_count, ic, cells, tc[256], vc[256];
  int            ih1, id1, stat, pc, vert_count1, tri_count1, tcm[3], tsvw;
  int            _offs[8], coffs[12], ctype[12], *ts, hep[3], planeidx[12], planeoff[2];
...
#include "tricases.h"
  ...
  *Tri_count = *Vert_count = 0;
  ...
  if ( _connectivity > 6 ) /* flip table up and down */
  {
    for ( i = 0; i < 128; i++ )
    {
      for ( j = 0; j < 19; j++ )
      {
        k = tricases[i][j];
        tricases[i][j] = tricases[255 - i][j];
        tricases[255 - i][j] = k;
      }
    }
  }
  if ( _connectivity == 26 ) /* modify table for 26 nbr _connectivity */
  {
    for ( i = 0; i < 19; i++ )
    {
  ...
  if ( _connectivity > 6 ) /* switch triangle orientation if we've flipped */
  {
    for ( i = 0; i < 256; i++ )
    {
      for ( j = 0; j < 18; j += 3 )
      {
        k = tricases[i][j];
        tricases[i][j] = tricases[i][j + 1];
        tricases[i][j + 1] = k;
      }
    }
  }

  _img_horiz = dims[0]; /* cols, fastest in memory */
  _img_vert = dims[1];  /* rows */
  _img_depth = dims[2]; /* planes, slowest in mem*/
  ...
  cells = id1 * cellplane;
  cellplanem[0] = 0;
  cellplanem[1] = cellplane;
  cellplanem[2] = ( cellplane << 1 );
  ...
  // std::cout << "cellplane: " << cellplane << std::endl;
  for ( i = 0; i < 256; i++ ) /* calculate number of tris, new verts for each
                                case */
  {
    hep[0] = hep[1] = hep[2] = 0;
    k = 0;
  ...
  coffs[0] = -ih1;
  ctype[0] = 1; /* cell edge midpoint type '6' */
  coffs[1] = 0;
  ctype[1] = 0; /* cell edge midpoint type '5' */
  coffs[2] = 0;
  ctype[2] = 1;
  coffs[3] = -1;
  ctype[3] = 0;
  ...
  coffs[8] = -ih1 - 1;
  ctype[8] = 2; /* cell edge midpoint type '11' */
  coffs[9] = -ih1;
  ctype[9] = 2;
  coffs[10] = -1;
  ctype[10] = 2;
  ...
  /* calculate _status of each cell */
  pidx = J + 1 + _img_horiz + ( _img_horiz * _img_vert ); /* begin at point
                                                            (1,1,1) */
  cidx = _status;                                         /* begin at cell
                                                            (0,0,0) */
  /* for each interior point */
  for ( k = _img_depth - 2; k >= 1; k--, pidx += ( _img_horiz << 1 ), cidx += ih1 )
  {
    for ( j = _img_vert - 2; j >= 1; j--, pidx += 2, cidx++ )
    {
      for ( i = _img_horiz - 2; i >= 1; i--, pidx++, cidx++ )
      {
        if ( *pidx == val )
        {
  ...
  *Tri_count = tri_count;
  ...
  for ( k = 0; k < id1; k++ ) /* for each cell plane */
  {
    ...
    pc = 0;                                    /* number of cell within cell
                                                 plane */
    for ( j = 0; j < iv1; j++ )                /* for each cell */
    {
      ...
      {
        stat = _status[ic];
        // std::cout << "Stat : " << stat << " ic: " << ic << std::endl;
        if ( tc[stat] ) /* no tris implies no verts */
        {
          // std::cout << "tricases[stat] : " << tricases[stat] << std::endl;
          ...
          for ( w = 0; w < 15; w++ )
          {
            ...
            else
            {
              ...
              {
                // std::cout << "ts[" << w << "] : " << ts[w] << " hep2[" << u
                // << "] :" <<  hep2[u] << std::endl;
                // std::cout << "hep[" << u << "] :" <<  hep[u] << std::endl;
                if ( ( ts[w] == hep2[u] ) && ( hep[u] == 0 ) )
                {
                  ...
                  // std::cout << "Vert : " << vert_count1;
                  // std::cout << " " << verts[vert_count1];
                  verts[vert_count1 + vert_count] = ( (float)j ) + half[u][0];
                  // std::cout << " " << verts[vert_count1+vert_count];
                  verts[vert_count1 + vert_count_times2] = ( (float)k ) + half[u][2];
                  // std::cout << " " << verts[vert_count1+vert_count_times2] <<
                  // std::endl;
                  vert_count1++;
                }
              }
          ...
          for ( w = 0; w < u; w += 3 ) /* for each triangle */
          {
            ...
            {
              tsvw = ts[v + w];
              tris[tri_count1 + tcm[v]] =
                v_idx[planeoff[planeidx[tsvw]] + pc + coffs[tsvw] + cellplanem[ctype[tsvw]]]; /*
                                                                                                whew!
                                                                                                */
            }
            tri_count1++;
  ...
  return 0; /* return with success _status */
} /*end!*/

static int
big_component( int * Tris, float * Verts, int * Vert_count, int * Tri_count )
{
  int     ov, ot, tri_count, vert_count, *v_count, *tp, *v_ran, *v_idx, u0, v0, i, j, *tris, w;
  int *   _que, _que_pos, _que_len, *component, _comp_count, *c_count, tc[3], k, qqp, vt, *tr;
  float * verts;
  ...
  if ( ( v_count = (int *)Gcalloc( vert_count, sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  ...
  if ( ( v_ran = (int *)Gcalloc( vert_count + 1, sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  ...
  if ( ( v_idx = (int *)Gcalloc( v_ran[vert_count], sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  ...
  if ( ( _que = (int *)Gcalloc( vert_count, sizeof( int ), 0 ) ) == nullptr )
  {
    error_msg( "Memory error.\n", __LINE__ );
    return 1;
  }
  ...
  for ( vt = 0; vt < vert_count; vt++ )
  {
    ...
    {
      _comp_count++; /* increase component count */
      // std::cout << "Increment _comp_count " << _comp_count << std::endl;
      _que_len = _que_pos = 0;                        /* reset que */
      component[_que[_que_len++] = vt] = _comp_count; /* add vt to start of que
                                                       */
      while ( _que_pos < _que_len )                   /* while que not empty */
      {
        qqp = _que[_que_pos];
        /* add nbrs to que */
        for ( i = v_ran[qqp]; i < v_ran[qqp + 1]; i++ )
        {
          ...
          for ( j = 0; j < 3; j++ )
          {
            ...
            if ( component[k] == 0 )
            {
              component[_que[_que_len++] = k] = _comp_count;
            }
          }
        }
        _que_pos++;
  ...
  if ( _comp_count > 1 )
  {
    if ( ( c_count = (int *)Gcalloc( 1 + _comp_count, sizeof( int ), 0 ) ) == nullptr )
    {
      error_msg( "Memory error.\n", __LINE__ );
      return 1;
    }
    ...
    for ( vt = 0; vt < vert_count; vt++ )
    {
      if ( component[vt] == k )
      {
        _que[vt] = j;         /* create remapping index */
    ...
    for ( vt = 0; vt < tri_count; vt++ )
    {
      ...
      {
        tris[w] = _que[tr[0]];
        tris[w + tc[1]] = _que[tr[tc[1]]];
        tris[w + tc[2]] = _que[tr[tc[2]]];
        w++;
      }
    }
  ...
  if ( tri_count != ot )
  {
    for ( i = 0; i < tri_count; i++ )
    {
      tris[tri_count + i] = tris[ot + i];
    }
    for ( i = 0; i < tri_count; i++ )
    {
      tris[( tri_count << 1 ) + i] = tris[( ot << 1 ) + i];
    }
  }
  ...
  if ( vert_count != ov )
  {
    for ( i = 0; i < vert_count; i++ )
    {
      verts[i + vert_count] = verts[i + ov];
    }
    for ( i = 0; i < vert_count; i++ )
    {
      verts[i + ( vert_count << 1 )] = verts[i + ( ov << 1 )];
    }
  }
...
static int
save_image( genus0parameters * g0 )
{
  ...
  if ( !( g0->any_genus ) ) /* if we made topological corrections */
  {
    ...
    if ( verbose )
    {
      sprintf( msg, "Made %d adjustments.\n", j );
      print_msg( msg );
    }
    Gfree( que );
    Gfree( cm );
  ...
  // std::cout << "Vert Count : " << g0->vert_count << std::endl;
  // std::cout << "Tri Count : " << g0->tri_count << std::endl;
  ...
  if ( g0->return_adjusted_label_map ) /* they want a new label map back */
  {
    ...
    if ( output == nullptr ) /* they didn't allocate.  So we need to */
    {
      if ( ( output = (unsigned short *)Gcalloc( origlen, sizeof( unsigned short ), 1 ) ) == nullptr )
      {
        error_msg( "Memory error.\n", __LINE__ );
        return 1;
      }
    ...
    for ( i = 0; i < origlen; i++ )
    {
      output[i] = ( g0->input )[i]; /* just copy original input
                                     */
    }
    if ( !( g0->any_genus ) )
    {
  ...
  vo[0] = 0;
  vo[1] = g0->vert_count;
  vo[2] = ( g0->vert_count ) * 2;
  verts = g0->vertices;

  if ( g0->return_adjusted_label_map ) /* they want a new label map back */
  {
    if ( g0->contour_value == 65535 )
    {
      g0->contour_value = g0->alt_value;
    }
    if ( g0->alt_contour_value == 65535 )
    {
      g0->alt_contour_value = g0->alt_value;
    }
    for ( h = 0; h < g0->vert_count; h++ )
    {
      ...
      for ( i = 0; i < 3; i++ )
      {
        ...
        if ( ( verts[h + vo[i]] - pos[i] ) == 0.5 )
        {
          j = i; /* must happen exactly once */
        }
      }
      ...
      if ( zpic[i] == 0 )
      {
        pos[j]++; /* subscript into boundary voxel of zpic */
      }
      for ( i = 0; i < 3; i++ )
      {
        pos[i] += autocrop[i][0] - pad[i]; /* subscript into
                                             boundary voxel of input
                                             */
      }
      i = sub2ind( pos[0], pos[1], pos[2], g0->dims[0], g0->dims[1] ); /* index
                                                                         into
                                                                         input */
  ...
  for ( h = 0; h < g0->vert_count; h++ )
  {
    for ( i = 0; i < 3; i++ )
    {
      verts[h + vo[i]] += autocrop[i][0] - pad[i];
    }
  }

  /* multiply verts by ijk2ras matrix */
  if ( g0->ijk2ras != nullptr )
  {
    m = g0->ijk2ras;
    for ( h = 0; h < g0->vert_count; h++ )
    {
...
extern void
genus0init( genus0parameters * g0 )
{
  g0->input = nullptr;
...
extern void
genus0destruct( genus0parameters * g0 )
{
  if ( g0->vertices != nullptr )
  {
    basic_free( g0->vertices );
    g0->vertices = nullptr;
  }
  if ( g0->triangles != nullptr )
  {
