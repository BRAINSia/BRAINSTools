...
DisplacementFieldTransform<TScalarType, NDimensions>::DisplacementFieldTransform()
  : Superclass(SpaceDimension, 0)
{
...
template <typename TScalarType, unsigned int NDimensions>
unsigned int
DisplacementFieldTransform<TScalarType, NDimensions>::GetNumberOfParameters(void) const
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::SetIdentity()
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::SetParameters(const ParametersType & parameters)
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::SetFixedParameters(const ParametersType & passedParameters)
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::WrapAsImages()
{
  /**
   ...
   */
  InternalPixelType * dataPointer = const_cast<InternalPixelType *>((m_InputParametersPointer->data_block()));
  unsigned int        numberOfParameters = this->GetNumberOfParameters();
  unsigned int        numberOfPixels = numberOfParameters / SpaceDimension;

  m_CoefficientImage->GetPixelContainer()->SetImportPointer(dataPointer, numberOfParameters);
  ...
  for (unsigned int j = 0; j < SpaceDimension; j++)
  {
    m_JacobianImage[j]->GetPixelContainer()->SetImportPointer(jacobianDataPointer, numberOfParameters);
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::SetParametersByValue(const ParametersType & parameters)
{
  // check if the number of parameters match the
  // expected number of parameters
  if (parameters.Size() != this->GetNumberOfParameters())
...
// Get the parameters
template <typename TScalarType, unsigned int NDimensions>
const typename DisplacementFieldTransform<TScalarType, NDimensions>::ParametersType &
DisplacementFieldTransform<TScalarType, NDimensions>::GetParameters(void) const
{
...
template <typename TScalarType, unsigned int NDimensions>
const typename DisplacementFieldTransform<TScalarType, NDimensions>::ParametersType &
DisplacementFieldTransform<TScalarType, NDimensions>::GetFixedParameters(void) const
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::SetImage(ImagePointer image)
{
  if (image)
  {
    ...
    // Clean up buffered parameters
    unsigned int numberOfParameters = this->GetNumberOfParameters();
    m_InternalParametersBuffer = ParametersType(image->GetPixelContainer()->GetImportPointer(), numberOfParameters);
    m_InputParametersPointer = &m_InternalParametersBuffer;
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::PrintSelf(std::ostream & os, Indent indent) const
{
  this->Superclass::PrintSelf(os, indent);

  os << indent << "CoefficientImage: " << m_CoefficientImage.GetPointer() << std::endl;

  os << indent << "InputParametersPointer: " << m_InputParametersPointer << std::endl;
}

// Transform a point
template <typename TScalarType, unsigned int NDimensions>
bool
DisplacementFieldTransform<TScalarType, NDimensions>::InsideValidRegion(const ContinuousIndexType & index) const
{
  bool inside = true;
...
DisplacementFieldTransform<TScalarType, NDimensions>::TransformPoint(const InputPointType & point) const
{
  unsigned int    j;
...
template <typename TScalarType, unsigned int NDimensions>
const typename DisplacementFieldTransform<TScalarType, NDimensions>::JacobianType &
DisplacementFieldTransform<TScalarType, NDimensions>::GetJacobian(const InputPointType & point)
{
...
template <typename TScalarType, unsigned int NDimensions>
void
DisplacementFieldTransform<TScalarType, NDimensions>::TransformPointToContinuousIndex(const InputPointType & point,
...
DisplacementFieldTransform<TScalarType, NDimensions>::LinearInterpolateAtIndex(const ContinuousIndexType & index) const
{
