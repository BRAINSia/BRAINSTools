  ...
  for ( unsigned int di = 0; di < NDimensions; di++ )
  {
    for ( unsigned int dj = 0; dj < NDimensions; dj++ )
    {
      this->m_FixedParameters[3 * NDimensions + ( di * NDimensions + dj )] =
        ( m_CoefficientImage->GetDirection() )[di][dj];
    }
  }
...
template < typename TScalarType, unsigned int NDimensions >
void
DisplacementFieldTransform< TScalarType, NDimensions >::TransformPointToContinuousIndex(
...
DisplacementFieldTransform< TScalarType, NDimensions >::LinearInterpolateAtIndex(
