...
{
bool
WriteEncodedDoubleVector( std::ostream & out, vnl_vector< double > & VectorBuffer )
{
  ...
  if ( compress( &compressedBuf[0], &compressedLength, srcBuffer, srcLength ) != Z_OK )
  {
    return false;
  }
  ...
  unsigned long                base64Len = itksysBase64_Encode( &compressedBuf[0], compressedLength, &base64Buf[0], 1 );

  std::copy( base64Buf.begin(), base64Buf.begin() + base64Len, std::ostream_iterator< unsigned char >( out ) );

  return true;
}

EncodedTransformFileWriter ::EncodedTransformFileWriter() {}

EncodedTransformFileWriter ::~EncodedTransformFileWriter() {}

/** Set the input transform and reinitialize the list of transforms */
void
EncodedTransformFileWriter::SetInput( const TransformType * transform )
{
...
void
EncodedTransformFileWriter::AddTransform( const TransformType * transform )
{
  m_TransformList.push_back( transform );
}
...
/** Update the writer */
void
EncodedTransformFileWriter ::Update()
{
