...
template <typename TInputImage, typename TOutputImage, typename TWeightPixelType>
void
GrowCutSegmentationImageFilter<TInputImage, TOutputImage, TWeightPixelType>::SetStateImage(const OutputImageType * l)
{
  ...
  std::vector<unsigned> & physicalVolumes)
{
  ...
  for (unsigned n = 0; n < numObjects; n++)
  {
    ...
    physicalVolumes[n] = (unsigned int)(labelFilter->GetOutput()->GetNthLabelObject(n)->GetNumberOfPixels() / 1000.0);
  }
#endif
}
  ...
  OutputImageType * stateImage)
{
  ...
  for (unsigned i = 0; i < labelImage->GetImageDimension(); i++)
  {
   ...
   (midROI[i]+m_ObjectRadius) : lsize[i]-1; //m_roiEnd[i];
    ...
    m_roiEnd[i] = (static_cast<unsigned int>(m_roiEnd[i] + m_ObjectRadius) < lsize[i]) ? (m_roiEnd[i] + m_ObjectRadius)
                                                                                       : lsize[i] - 1;
  }
