...
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GrowCutSegmentationImageFilter()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::PrintSelf( std::ostream & os,
                                                                                          Indent         indent ) const
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GenerateInputRequestedRegion()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::EnlargeOutputRequestedRegion(
  DataObject * output )
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::SetStateImage(
  const OutputImageType * l )
{
...
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetStateImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::SetDistancesImage(
  const WeightImageType * w )
{
...
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetDistancesImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::SetMaxSaturationImage(
  const WeightImageType * s )
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
const typename GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::WeightImagePointer
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetMaxSaturationImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
const typename GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::OutputImagePointer
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetLabelImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
const typename GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::WeightImagePointer
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetStrengthImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
const typename GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::WeightImagePointer
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetUpdatedStrengthImage()
{
...
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GetInputImage()
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::Initialize( OutputImageType * output )
{
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::ComputeLabelVolumes(
  TOutputImage * outputImage, std::vector< unsigned > & volumes, std::vector< unsigned > & physicalVolumes )
{
  // volumes.resize(3);
  ...
  ImageRegionConstIteratorWithIndex< OutputImageType > label( outputImage, outputImage->GetBufferedRegion() );
  ...
  {
    volumes[n] = labelFilter->GetOutput()->GetNthLabelObject( n )->GetNumberOfPixels();
    physicalVolumes[n] =
      (unsigned int)( labelFilter->GetOutput()->GetNthLabelObject( n )->GetNumberOfPixels() / 1000.0 );
  }
#endif
}
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
bool
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::InitializeStateImage(
  OutputImageType * stateImage )
{
  ...
  typename OutputImageType::SizeType lsize = labelImage->GetBufferedRegion().GetSize();

  ImageRegionConstIteratorWithIndex< OutputImageType > label( labelImage, labelImage->GetBufferedRegion() );
  ...
  for ( label.GoToBegin(), state.GoToBegin(); !label.IsAtEnd(); ++label, ++state )
  {
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::InitializeDistancesImage(
  TInputImage * inputImage, WeightImageType * distances )
{
  ...
  InputNeighborhoodIteratorType input( radiusIn, inputImage, inputImage->GetBufferedRegion() );


  ImageRegionIterator< WeightImageType > dist( distances, distances->GetBufferedRegion() );
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::MaskSegmentedImageByWeight(
  float confThresh )
{
  ImageRegionConstIterator< WeightImageType > weight( m_WeightImage, m_WeightImage->GetBufferedRegion() );

  ImageRegionIterator< OutputImageType > label( m_LabelImage, m_LabelImage->GetBufferedRegion() );
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::GenerateData()
{
  ...
  // Filter was configured to run until convergence. We need to delegate a different instance of the filter to run on
  // each iteration. std::cout<<" Running filter until convergence .... "<<std::endl;

  typename GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::Pointer singleIteration =
  ...
  singleIteration->GetOutput()->SetBufferedRegion( this->GetOutput()->GetBufferedRegion() );
  ...
  while ( iter < m_MaxIterations && !converged )
  {
    ...
    unsigned changeablePix = singleIteration->GetLabeled(); // updateFilter->GetNumLabeledPixels();
    unsigned currLocallySaturatedPix =
      singleIteration->GetLocallySaturated();                    // updateFilter->GetNumLocallySaturatedPixels();
    unsigned currSaturatedPix = singleIteration->GetSaturated(); // updateFilter->GetNumSaturatedPixels();

    unsigned unlabeled =
      ( changeablePix + currLocallySaturatedPix + currSaturatedPix <= totalROIVolume )
        ? ( static_cast< unsigned >( totalROIVolume ) - ( currLocallySaturatedPix + currSaturatedPix + changeablePix ) )
        : 0;
    ...
    // ((currModified-prevModifiedPix)*100)/(std::max(static_cast<unsigned int>(1), std::max(currModified,
    // prevModifiedPix))) : ((prevModifiedPix - currModified)*100)/(std::max(static_cast<unsigned int>(1),
    // std::max(currModified, prevModifiedPix)));

    converged =
      ( prevModifiedPix == currModified ) || ( relUnlabeled < threshUnchanged && relSaturated > threshSaturation );
    ...
    if ( !converged )
    {
      ...
      singleIteration->GetOutput()->SetBufferedRegion( output->GetBufferedRegion() );
    }
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::ThreadedGenerateData(
  const OutputImageRegionType & outputRegionForThread, ThreadIdType itkNotUsed( threadId ) )
{
  ...
  for ( fitIn = faceList.begin(); fitIn != faceList.end(); ++fitIn )
  {
    ...
    inputIt.GoToBegin();
    oldLabelIt.GoToBegin();
    newLabelIt.GoToBegin();
    oldWeightIt.GoToBegin();
    newWeightIt.GoToBegin();
    stateIt.GoToBegin();
    // newstateIt.GoToBegin();
    distancesIt.GoToBegin();
    maxSatIt.GoToBegin();
...
template < typename TInputImage, typename TOutputImage, typename TWeightPixelType >
void
GrowCutSegmentationImageFilter< TInputImage, TOutputImage, TWeightPixelType >::AfterThreadedGenerateData()
{
  m_Labeled = 0;
