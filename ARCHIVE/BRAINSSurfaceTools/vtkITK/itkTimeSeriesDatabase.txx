...
// template<typename TPixel> int TimeSeriesDatabase<TPixel>::BlockSize = 16;
template < typename T >
T
TSD_MIN( T a, T b )
{
                                                          ...
                                                          typename OutputImageType::RegionType & ImageRegion )
{
  ...
  {
    ImageRegion.SetIndex(
      i,
      TSD_MAX< itk::IndexValueType >( RequestedRegion.GetIndex( i ),
...
template < typename TPixel >
bool
TimeSeriesDatabase< TPixel >::IsOpen() const
{
  if ( this->m_DatabaseFiles.size() == 0 )
  {
    return false;
  }
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::Disconnect()
{
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::Connect( const char * filename )
{
  ...
  if ( version != 1.0 )
  {
    itkExceptionMacro( "TimeSeriesDatabase::Connect: Version string does not match.  Expecting 1.0, found "
                       << version );
  }
  ...
  for ( int idx = 0; idx < NumberOfFiles; idx++ )
  {
    ...
    this->m_DatabaseFiles.push_back(
      StreamPtr( new std::fstream( Filename.c_str(), ::std::ios::in | ::std::ios::binary ) ) );
  }
  /*
...
template < typename TPixel >
unsigned int
TimeSeriesDatabase< TPixel >::CalculateFileIndex( unsigned long Index, unsigned long BlocksPerFile )
{
...
template < typename TPixel >
unsigned int
TimeSeriesDatabase< TPixel >::CalculateFileIndex( unsigned long index )
{
                                                            ...
                                                            unsigned int BlocksPerImage[3] )
{
  ...
  unsigned long index = 1 + p[0] + p[1] * BlocksPerImage[0] + p[2] * BlocksPerImage[0] * BlocksPerImage[1] +
...
template < typename TPixel >
::std::streampos
TimeSeriesDatabase< TPixel >::CalculatePosition( unsigned long index, unsigned long BlocksPerFile )
{
...
template < typename TPixel >
typename TimeSeriesDatabase< TPixel >::CacheBlock *
TimeSeriesDatabase< TPixel >::GetCacheBlock( unsigned long index )
{
  ...
  if ( Buffer == nullptr )
  {
    ...
    this->m_DatabaseFiles[FileIdx]->read( reinterpret_cast< char * >( B.data ),
                                          TimeSeriesVolumeBlockSize * sizeof( TPixel ) );
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::GetVoxelTimeSeries( typename OutputImageType::IndexType idx, ArrayType & array )
{
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::GenerateOutputInformation()
{
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::GenerateData()
{
  typename OutputImageType::Pointer    output = this->GetOutput();
  ...
  for ( unsigned int i = 0; i < 3; i++ )
  {
    ...
    BlockCount[i] = (int)TSD_MAX(
      1.0, ceil( ( Region.GetIndex( i ) + Region.GetSize( i ) ) / (double)TimeSeriesBlockSize ) - BlockStart[i] );
  }
            ...
            for ( y = 0; y < Count[1]; y++ )
            {
              ...
              for ( x = 0; x < Count[0]; x++ )
              {
                ...
                if ( ImageIndex[0] == 45 && ImageIndex[1] == 0 && ImageIndex[2] == 0 ) {
                  std::cout << "Index: " << ImageIndex << " Volume Value: " << output->GetPixel ( ImageIndex ) << "
                buffer: " << Buffer->data[BufferIndex] << std::endl; std::cout << "Index: " << ImageIndex << " From " <<
                BufferIndex << " ( " << bx << ", " << by << ", " << bz << " )\n" << std::endl;
                }
                */

                output->SetPixel(
                  ImageIndex,
                  Buffer->data[bx + TimeSeriesBlockSize * by + TimeSeriesBlockSize * TimeSeriesBlockSize * bz] );
              }
            }
          }
        }
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::CreateFromFileArchetype( const char * TSDFilename, const char * archetype )
{
                                                       ...
                                                       unsigned long FileSize )
{
  ...
  {
    itkGenericExceptionMacro( "TimeSeriesDatabase::CreateFromFileArchetype: Archetype file "
  ...
  for ( unsigned int i = 0; i < candidateFiles.size(); i++ )
  {
    ...
    region = reader->GetOutput()->GetLargestPossibleRegion();
    if ( m_Dimensions[0] != region.GetSize()[0] || m_Dimensions[1] != region.GetSize()[1] ||
         m_Dimensions[2] != region.GetSize()[2] )
    {
      ...
      itkGenericExceptionMacro( << " size of the data in " << candidateFiles[i] << " is (" << region.GetSize()[0]
                                << ", " << region.GetSize()[1] << ", " << region.GetSize()[2] << ") "
                                << " and does not match the expectected size (" << m_Dimensions[0] << ", "
    ...
    for ( CurrentBlock[2] = 0; CurrentBlock[2] < m_BlocksPerImage[2]; CurrentBlock[2]++ )
    {
      for ( CurrentBlock[1] = 0; CurrentBlock[1] < m_BlocksPerImage[1]; CurrentBlock[1]++ )
      {
        ...
        {
          /*
          std::cout << "Reading/Writing Block: " << CurrentBlock[0] << ", " << CurrentBlock[1] << ", " <<
          CurrentBlock[2] << endl; std::cout << "Debug: " << ( ( CurrentBlock[2] * TimeSeriesBlockSize +
          TimeSeriesBlockSize ) < region.GetSize()[2] ) << endl; std::cout << "Debug: " << CurrentBlock[2] *
          TimeSeriesBlockSize + TimeSeriesBlockSize << " Region size  " << region.GetSize()[2] << endl;
          */
            ...
            for ( unsigned int bz = StartIndex[2]; bz < EndIndex[2]; bz++ )
            {
              ...
              for ( unsigned int by = StartIndex[1]; by < EndIndex[1]; by++ )
              {
                ...
                for ( unsigned int bx = StartIndex[0]; bx < EndIndex[0]; bx++ )
                {
                  ...
                  buffer[bx - StartIndex[0] + TimeSeriesBlockSize * ( by - StartIndex[1] ) +
                         TimeSeriesBlockSize * TimeSeriesBlockSize * ( bz - StartIndex[2] )] = value;
                }
              }
            }
          }
          ...
          // std::cout << "Found FileIndex : " << FileIndex << " For index: " << index << " position: " << position <<
          // std::endl;
  ...
  b << "Dimensions: " << m_Dimensions[0] << " " << m_Dimensions[1] << " " << m_Dimensions[2] << " " << m_Dimensions[3]
    << std::endl;
...
template < typename TPixel >
float
TimeSeriesDatabase< TPixel >::GetCacheSizeInMiB()
{
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::SetCacheSizeInMiB( float sz )
{
...
template < typename TPixel >
void
TimeSeriesDatabase< TPixel >::PrintSelf( std::ostream & os, Indent indent ) const
{
