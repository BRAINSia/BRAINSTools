                                                       ...
                                                       typename OutputImageType::RegionType & ImageRegion)
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    Tmp = TSD_MIN<itk::IndexValueType>(Tmp, static_cast<IndexValueType>(TimeSeriesBlockSize * (BlockIndex[i] + 1)));
...
TimeSeriesDatabase<TPixel>::GenerateData()
{
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    ...
    BlockCount[i] =
      (int)TSD_MAX(1.0, ceil((Region.GetIndex(i) + Region.GetSize(i)) / (double)TimeSeriesBlockSize) - BlockStart[i]);
  }
...
template <typename TPixel>
void
TimeSeriesDatabase<TPixel>::CreateFromFileArchetype(const char *  TSDFilename,
                                                    ...
                                                    unsigned long FileSize)
{
  ...
  {
    itkGenericExceptionMacro("TimeSeriesDatabase::CreateFromFileArchetype: Archetype file " << fileNameCollapsed.c_str()
                                                                                            << " does not exist.");
    return;
  }
