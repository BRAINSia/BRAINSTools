...
IccdefPreprocessor<TInputImage, TOutputImage>::IccdefPreprocessor()
{
...
template <typename TInputImage, typename TOutputImage>
void
IccdefPreprocessor<TInputImage, TOutputImage>::Execute()
{
  ...
  if (this->m_FixedBinaryVolume != std::string("none"))
  {
    if (this->GetOutDebug())
    {
      ...
      std::cout << "PRE Fixed Origin" << m_InputFixedImage->GetOrigin() << std::endl;
    }
    m_InputFixedImage = this->MakeBOBFImage(m_InputFixedImage, m_FixedBinaryVolume);
    if (this->GetOutDebug())
    {
      std::cout << "Fixed Origin" << m_InputFixedImage->GetOrigin() << std::endl;
      std::cout << "PRE Moving Origin" << m_InputMovingImage->GetOrigin() << std::endl;
    }
    m_InputMovingImage = this->MakeBOBFImage(m_InputMovingImage, m_MovingBinaryVolume);
    if (this->GetOutDebug())
    {
      std::cout << "Moving Origin" << m_InputMovingImage->GetOrigin() << std::endl;
  ...
  if (m_InitialTransformFilename != "")
  {
    ...
    if (transforms->size() == 0)
    {
      ...
      std::cout << "FATAL ERROR: Failed to read transform" << m_InitialTransformFilename << std::endl;
      exit(-1);
    }
    ...
    m_InitialDisplacementField =
      itkUtil::OrientImage<TDisplacementField>(m_InitialDisplacementField, m_InputFixedImage->GetDirection());
  ...
  m_InputFixedImage =
    itkUtil::OrientImage<InputImageType>(m_InputFixedImage, itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
  m_InputMovingImage =
    itkUtil::OrientImage<InputImageType>(m_InputMovingImage, itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
  ...
  if (this->GetOutDebug())
  {
    ...
    itkUtil::WriteImage<TOutputImage>(m_OutputFixedImage, "HistogramReferenceFixedImage.nii.gz");
    itkUtil::WriteImage<TOutputImage>(m_OutputMovingImage, "HistogramModifiedMovingImage.nii.gz");
  }
...
template <typename TInputImage, typename TOutputImage>
typename IccdefPreprocessor<TInputImage, TOutputImage>::InputImagePointer
IccdefPreprocessor<TInputImage, TOutputImage>::MakeBOBFImage(InputImagePointer input, std::string MaskName)
{
