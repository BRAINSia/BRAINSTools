...
void
IccdefRegistrator<TRealImage, TOutputImage, TFieldValue>::WriteDisplacementComponents(TDisplacementField * df,
                                                                                      std::string          prefix)
{
  ...
  {
    std::cerr << "exception in file Displacement File Writer(" << CurrentComponentFilename << ")" << std::endl;
...
IccdefRegistrator<TRealImage, TOutputImage, TFieldValue>::IccdefRegistrator()
{
  ...
  typename FieldInterpolatorType::Pointer VectorInterpolator12 = FieldInterpolatorType::New();
...
template <typename TRealImage, typename TOutputImage, typename TFieldValue>
void
IccdefRegistrator<TRealImage, TOutputImage, TFieldValue>::Execute()
{
  ...
  this->m_Registration->SetNumberOfIterations(this->m_NumberOfIterations.data_block());
  ...
  try
  {
    ...
    if (this->m_DisplacementField->GetDirection() != this->m_FixedImage->GetDirection())
    {
      std::cout << "ERROR Directions don't match\n"
                ...
                << this->m_FixedImage->GetDirection() << std::endl;
      exit(-1);
    }
    if (this->m_Tag)
    {
  ...
  {
    itkUtil::WriteImage<TDisplacementField>(this->m_DisplacementField, this->m_ForwardDisplacementFieldOutputName);
    if (this->GetOutDebug())
    {
      std::cout << "---Forward deformation field has been written " << this->m_ForwardDisplacementFieldOutputName
                << "--" << std::endl;
    }
  }
      ...
      this->m_BackwardDisplacementFieldOutputName != std::string(""))
  {
    ...
    if (this->GetOutDebug())
    {
      std::cout << "---Backward deformation field has been written " << this->m_BackwardDisplacementFieldOutputName
                << "--" << std::endl;
    }
  }
#endif
  ...
  if (this->GetOutputJacobianImage())
  {
    using JacobianFilterType =
      itk::DisplacementFieldJacobianDeterminantFilter<TDisplacementField, FieldValueType, TRealImage>;
  ...
  if (this->m_OutputPrefix != std::string("none"))
  {
    ...
    using WarperType = WarpImageFilter<RealImageType, RealImageType, TDisplacementField>;
...
template <typename TRealImage, typename TOutputImage, typename TFieldValue>
void
IccdefRegistrator<TRealImage, TOutputImage, TFieldValue>::StartNewLevel()
{
  if (this->GetOutDebug())
  {
    std::cout << "--- Starting level " << this->m_Registration->GetCurrentLevel() << std::endl;
  }
}
} // namespace itk
#endif
