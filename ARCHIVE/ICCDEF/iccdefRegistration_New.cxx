...
void
ThirionFunction(const struct ICCDEFWarpAppParameters & command)
{
  ...
  typedef typename itk::ICCDEFWarp<ImageType, TRealImage, OutputImageType> AppType;
  ...
  using ActualRegistrationFilterType =
    typename itk::ICCDeformableRegistrationFilter<TRealImage, TRealImage, TDisplacementField>;
  ActualRegistrationFilterType::Pointer filter = ActualRegistrationFilterType::New();
  ...
  if (command.useConsistentLandmark)
  {
    ...
    {
      std::cout << "ERROR:  Must specify landmark file names when usingConsistentLandmark is set to true" << std::endl;
      exit(-1);
    }
    else
    {
      ...
      fixedVolume =
        itkUtil::OrientImage<TRealImage>(fixedVolume, itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
      typename TRealImage::Pointer movingVolume = itkUtil::ReadImage<TRealImage>(command.movingVolume.c_str());
      movingVolume =
        itkUtil::OrientImage<TRealImage>(movingVolume, itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RAI);
  ...
  {
    std::cout << "ERROR:  Must specify either Consistent intensity or Consistent landmark!" << std::endl;
    exit(-1);
  }


  typename CommandIterationUpdate<float, 3>::Pointer observer = CommandIterationUpdate<float, 3>::New();
  ...
  {
    app->SetInitialFixedDisplacementFieldFilename(command.initialFixedDisplacementFieldVolume.c_str());
  }

  if (command.initialMovingDisplacementFieldVolume != "")
  {
    app->SetInitialMovingDisplacementFieldFilename(command.initialMovingDisplacementFieldVolume.c_str());
  }
  ...
  if (command.maskProcessingMode == "BOBF")
  {
    ...
    {
      std::cout << "Error: If BOBF option is set for maskProcessingMode then the fixed mask name and moving mask file "
...
template <typename InPixelType, typename OutPixelType>
void
ProcessAppType(const struct ICCDEFWarpAppParameters & command)
{
...
main(int argc, char * argv[])
{
  ...
  if (command.outputDebug)
  {
    std::cout << "                        movingVolume: " << command.movingVolume << std::endl
              ...
              << "                     outputDirectory: " << command.outputDirectory << std::endl
              << "              useConsistentIntensity: " << command.useConsistentIntensity << std::endl
              << "               useConsistentLandmark: " << command.useConsistentLandmark << std::endl
              << " outputForwardDisplacementFieldVolume: " << command.outputForwardDisplacementFieldVolume << std::endl
              << "outputBackwardDisplacementFieldVolume: " << command.outputBackwardDisplacementFieldVolume << std::endl
              << "                        outputPrefix: " << command.outputPrefix << std::endl
              ...
              << "                    medianFilterSize: " << command.medianFilterSize << std::endl
              << "  initialFixedDisplacementFieldVolume: " << command.initialFixedDisplacementFieldVolume << std::endl
              << " initialMovingDisplacementFieldVolume: " << command.initialMovingDisplacementFieldVolume << std::endl
              ...
              << "                       fixedLandmark: " << command.fixedLandmark << std::endl
              << "                      movingLandmark: " << command.movingLandmark << std::endl
