...
template < typename TInputImage, typename TOutputImage >
void
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::GenerateData()
{
  TimeType time;
...
template < typename TInputImage, typename TOutputImage >
void
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::ComputeInverse(
  InputImageConstPointer & inputPtr, OutputImagePointer & outputPtr )
{
  ThreadStruct str;
...
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::ComputeInverseThreaderCallback(
  void * arg )
{
  ThreadStruct * str;
...
template < typename TInputImage, typename TOutputImage >
void
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::ThreadedComputeInverse(
  InputImageConstPointer & inputPtr, OutputImagePointer & outputPtr, const ThreadRegionType & regionToProcess, int )
{
    ...
    float               fDispU1, fDispU2, fDispU3;
    {
      ...
      do
      {
        ...
        {
          {
            ...
            typename InputImageType::PixelType pixel =
              TrilinearInterpolationFast( fullDestPnt1, fullDestPnt2, fullDestPnt3, size );
            fDispU1 = pixel[0];
            fDispU2 = pixel[1];
            fDispU3 = pixel[2];
          }
          ...
          iIteration++;
                    ...
                    ( std::abs( fDelta3 ) > fEpsilon3 ) ) &&
                  ( iIteration < current_check_level ) );
        ...
        current_check_level = current_check_level + OnePercentIncrementOfMaxIterations;
                  ...
                  ( std::abs( fDelta3 ) > fEpsilon3 ) ) &&
                ( iIteration < m_NumberOfIterations ) );
    }
...
template < typename TInputImage, typename TOutputImage >
typename ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::OutputImagePixelType
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::TrilinearInterpolationFast(
  float & fDesiredX, float & fDesiredY, float & fDesiredZ, InputImageSizeType size )
{
#if defined( __sgi )
...
#endif
  {
    ...
    if ( ( iFlooredX > 0 ) && ( iFlooredX < nx - 1 ) && ( iFlooredY > 0 ) && ( iFlooredY < ny - 1 ) &&
         ( iFlooredZ > 0 ) && ( iFlooredZ < nz - 1 ) )
    {
...
template < typename TInputImage, typename TOutputImage >
typename ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::InputImageIndexType
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::BoundaryIndexing(
  int iDesiredX, int iDesiredY, int iDesiredZ, const int iMaxX, const int iMaxY, const int iMaxZ )
{
  iDesiredZ = ( ( iDesiredZ > -1 ) ? ( ( iDesiredZ < iMaxZ ) ? iDesiredZ : ( iMaxZ - 1 ) ) : ( 0 ) );
  iDesiredY = ( ( iDesiredY > -1 ) ? ( ( iDesiredY < iMaxY ) ? iDesiredY : ( iMaxY - 1 ) ) : ( 0 ) );
  iDesiredX = ( ( iDesiredX > -1 ) ? ( ( iDesiredX < iMaxX ) ? iDesiredX : ( iMaxX - 1 ) ) : ( 0 ) );
...
template < typename TInputImage, typename TOutputImage >
void
ICCIterativeInverseDisplacementFieldImageFilter< TInputImage, TOutputImage >::PrintSelf( std::ostream & os,
                                                                                         Indent         indent ) const
{
