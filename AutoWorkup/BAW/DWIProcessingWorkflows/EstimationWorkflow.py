...
def create_estimation_workflow(WFname):
    ...
    def run_dti_process(dti_image):
        ...
        assert os.path.isfile(frobenius_norm_output), (
            "frobenius_norm file is not found: %s" % frobenius_norm_output
        )
        assert os.path.isfile(lambda1_output), (
            "lambda1 file is not found: %s" % lambda1_output
        )
        assert os.path.isfile(lambda2_output), (
            "lambda2 file is not found: %s" % lambda2_output
        )
        assert os.path.isfile(lambda3_output), (
            "lambda3 file is not found: %s" % lambda3_output
        )
        return (
            fa_output,
            md_output,
    ...
    inputsSpec = pe.Node(
        interface=IdentityInterface(fields=["inputDWIImage", "DWIBrainMask"]),
        name="inputsSpec",
    )
