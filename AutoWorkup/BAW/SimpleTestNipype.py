...
DWI_AutoProcess = pe.Workflow(
    name="DWI_20130515"
)  # HACK: This needs to be specified in the config file.
...
DWI_DG = pe.Node(
    interface=nio.DataGrabber(
        sort_filelist=False,
        outfields=["DWIQCed", "ACPCT1", "ACPCT2", "FST1", "FSWMParc"],
    ),
...
DWI_ResampleInPlace = pe.Node(
    interface=gtractResampleDWIInPlace(), name="DWI_ResampleInPlace"
)
...
DWI_AutoProcess.connect(
    BRAINSFitDWI2ACPC, "outputTransform", DWI_ResampleInPlace, "inputTransform"
)
...
ResampleWMParc2DWIACPC = pe.Node(
    interface=BRAINSResample(), name="ResampleWMParc2DWIACPC"
)
...
DWI_AutoProcess.connect(
    BRAINSFitFS2ACPC, "outputTransform", ResampleWMParc2DWIACPC, "warpTransform"
)
DWI_AutoProcess.connect(DWI_DG, "FSWMParc", ResampleWMParc2DWIACPC, "inputVolume")
DWI_AutoProcess.connect(
    DWI_ResampleInPlace, "outputVolume", ResampleWMParc2DWIACPC, "referenceVolume"
)
...
DWI_AutoProcess.connect(
    BRAINSFitFS2ACPC, "outputTransform", ResampleWMParc2ACPC, "warpTransform"
)
