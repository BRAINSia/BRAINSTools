...
class FSScriptInputSpec(CommandLineInputSpec):
    ...
    subjects_dir = Directory(
        argstr="--subjects_dir %s", desc="FreeSurfer subjects directory"
    )
...
class FSScriptOutputSpec(TraitedSpec):
    ...
    processed_output_name = traits.Str(
        desc="The name of the subdirectory (not a full path) for this processing stage"
    )
    outDir = Directory(
        exist=True,
        desc="Full path to the output directory for this stage of processing",
    )
    ## HACK: TEST
    subj_session_id = traits.Str(
        desc='Subject_Session used for "-subjid <> in cross sectional and used in -long <> for longitudinal'
...
class FSScript(CommandLine):
    ...
    def _list_outputs(self):
        ...
        if self.inputs.subcommand == "autorecon":
            outputs["T1_out"] = os.path.join(
                self.inputs.subjects_dir, "mri", "brain.mgz"
            )
            outputs["label1_out"] = os.path.join(
                self.inputs.subjects_dir, "mri_nifti", "aparc+aseg.nii.gz"
            )
            outputs["label2_out"] = os.path.join(
                self.inputs.subjects_dir, "mri_nifti", "aparc.a2009+aseg.nii.gz"
            )
            outputs["processed_output_name"] = self.inputs.subj_session_id
            outputs["outDir"] = os.path.join(
                self.inputs.subjects_dir, self.inputs.subj_session_id
            )
        elif self.inputs.subcommand == "template":
            outputs["processed_output_name"] = self.inputs.base_template_id
            outputs["outDir"] = os.path.join(
                self.inputs.subjects_dir, self.inputs.base_template_id
            )
        elif self.inputs.subcommand == "longitudinal":
            longitudinal_processed_output_name = (
                self.inputs.subj_session_id + ".long." + self.inputs.base_template_id
            )
            outputs["processed_output_name"] = longitudinal_processed_output_name
            outputs["outDir"] = os.path.join(
                self.inputs.subjects_dir, longitudinal_processed_output_name
            )
        return outputs
