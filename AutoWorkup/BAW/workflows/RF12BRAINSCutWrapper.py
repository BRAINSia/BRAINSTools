...
class RF12BRAINSCutWrapperCLInputSpec(CommandLineInputSpec):
    ...
    ### subject specific
    inputSubjectT1Filename = File(
        desc="Subject T1 Volume",
        exists=True,
        mandatory=True,
        argstr="--inputSubjectT1Filename %s",
    )
    inputSubjectT2Filename = File(
        desc="Subject T2 Volume",
        exists=True,
        mandatory=False,
        argstr="--inputSubjectT2Filename %s",
    )
    candidateRegion = File(
        desc="Valid region for finding GM structures",
        exists=True,
        mandatory=True,
        argstr="--candidateRegion %s",
    )
    inputSubjectGadSGFilename = File(
        desc="Subject SG Volume",
        exists=True,
        mandatory=False,
    ...
    ### model specific
    modelFilename = File(
        desc="modelFilename", exists=True, mandatory=True, argstr="--modelFilename %s"
    )
    trainingVectorFilename = File(
        desc="training vectof file name",
        exists=False,
        mandatory=False,
        argstr="--trainingVectorFilename %s",
    )
    inputTemplateT1 = File(
        desc="Atlas Template T1 image",
        exists=False,
        mandatory=False,
        argstr="--inputTemplateT1 %s",
    )
    inputTemplateRhoFilename = File(
        desc="Atlas Template rho image",
        exists=False,
        mandatory=False,
        argstr="--inputTemplateRhoFilename %s",
    )
    inputTemplatePhiFilename = File(
        desc="Atlas Template phi image",
        exists=False,
        mandatory=False,
        argstr="--inputTemplatePhiFilename %s",
    )
    inputTemplateThetaFilename = File(
        desc="Atlas Template theta image",
        exists=False,
        mandatory=False,
    ...
    xmlFilename = File(
        desc="Net configuration xml file",
        exists=False,
        mandatory=False,
        argstr="--xmlFilename %s",
    )

    outputBinaryLeftAccumben = File(
        desc="Output binary file of left accumben",
        exists=False,
        mandatory=True,
    ...
    outputBinaryLeftCaudate = File(
        desc="Output binary file of left caudate",
        exists=False,
        mandatory=True,
        argstr="--outputBinaryLeftCaudate %s",
    )
    outputBinaryRightCaudate = File(
        desc="Output binary file of right caudate",
        exists=False,
        mandatory=True,
        argstr="--outputBinaryRightCaudate %s",
    )

    outputBinaryLeftGlobus = File(
        desc="Output binary file of left globus",
        exists=False,
        mandatory=True,
        argstr="--outputBinaryLeftGlobus %s",
    )
    outputBinaryRightGlobus = File(
        desc="Output binary file of right globus",
        exists=False,
        mandatory=True,
    ...
    outputBinaryLeftPutamen = File(
        desc="Output binary file of left putamen",
        exists=False,
        mandatory=True,
        argstr="--outputBinaryLeftPutamen %s",
    )
    outputBinaryRightPutamen = File(
        desc="Output binary file of right putamen",
        exists=False,
        mandatory=True,
        argstr="--outputBinaryRightPutamen %s",
    )

    outputBinaryLeftThalamus = File(
        desc="Output binary file of left thalamus",
        exists=False,
        mandatory=True,
...
class RF12BRAINSCutWrapperCLOutputSpec(TraitedSpec):
    ...
    outputBinaryLeftAccumben = File(
        desc="Output binary file of left accumben", exists=True, mandatory=True
    )
    outputBinaryRightAccumben = File(
        desc="Output binary file of right accumben", exists=True, mandatory=True
    )

    outputBinaryLeftCaudate = File(
        desc="Output binary file of left caudate", exists=True, mandatory=True
    )
    outputBinaryRightCaudate = File(
        desc="Output binary file of right caudate", exists=True, mandatory=True
    )

    outputBinaryLeftGlobus = File(
        desc="Output binary file of left globus", exists=True, mandatory=True
    )
    outputBinaryRightGlobus = File(
        desc="Output binary file of right globus", exists=True, mandatory=True
    )

    outputBinaryLeftHippocampus = File(
        desc="Output binary file of left hippocampus", exists=True, mandatory=True
    )
    outputBinaryRightHippocampus = File(
        desc="Output binary file of right hippocampus", exists=True, mandatory=True
    )

    outputBinaryLeftPutamen = File(
        desc="Output binary file of left putamen", exists=True, mandatory=True
    )
    outputBinaryRightPutamen = File(
        desc="Output binary file of right putamen", exists=True, mandatory=True
    )

    outputBinaryLeftThalamus = File(
        desc="Output binary file of left thalamus", exists=True, mandatory=True
    )
    outputBinaryRightThalamus = File(
        desc="Output binary file of right thalamus", exists=True, mandatory=True
    )
            ...
            if isdefined(coresponding_input):
                if isinstance(coresponding_input, bool) and coresponding_input == True:
                    ...
                    if isinstance(coresponding_input, list):
                        outputs[name] = [
                            os.path.abspath(inp) for inp in coresponding_input
                        ]
                    else:
