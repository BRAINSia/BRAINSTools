...
def BAWantsRegistrationTemplateBuildSingleIterationWF(iterationPhasePrefix=''):
    ...
    BeginANTS.inputs.transforms = ["Rigid","Affine","SyN","SyN"]
    ...
    BeginANTS.inputs.sampling_percentage = [0.27,0.27,1.0,1.0]
    BeginANTS.inputs.metric_weight = [1.0,1.0,1.0,1.0]
    BeginANTS.inputs.radius_or_number_of_bins = [32,32,4,4]
    BeginANTS.inputs.number_of_iterations = [[1000,1000,1000,1000],[1000,1000,1000,1000],[1000,250],[500,70]]
    BeginANTS.inputs.convergence_threshold = [5e-8,5e-8,5e-7,1e-6]
