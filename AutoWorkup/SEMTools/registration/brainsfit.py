...
class BRAINSFitInputSpec(CommandLineInputSpec):
    ...
    metricSamplingStrategy = traits.Enum("None", "Random", "Regular", desc="It defines the method that registration filter uses to sample the input fixed image={None,Regular,Random}", argstr="--metricSamplingStrategy %s")
    ...
    samplingPercentage = traits.Float(desc="This is a number in [0,1] interval that shows the percentage of the input fixed image voxels that are sampled for mutual information computation.  Increase this for a slower, more careful fit. You can also limit the sampling focus with ROI masks and ROIAUTO mask generation.", argstr="--samplingPercentage %f")
    ...
    transformType = InputMultiPath(traits.Str, desc="Specifies a list of registration types to be used.  The valid types are, Rigid, ScaleVersor3D, ScaleSkewVersor3D, Affine, BSpline and SyN.  Specifiying more than one in a comma separated list will initialize the next stage with the previous results. If registrationClass flag is used, it overrides this parameter setting.", sep=",", argstr="--transformType %s")
    ...
    initializeRegistrationByCurrentGenericTransform = traits.Bool(desc="If this flag is ON, the current generic composite transform, resulted from the linear registration stages, is set to initialize the follow nonlinear registration process. However, by the default behaviour, the moving image is first warped based on the existant transform before it is passed to the BSpline registration filter. It is done to speed up the BSpline registration by reducing the computations of composite transform jacobian.", argstr="--initializeRegistrationByCurrentGenericTransform ")
