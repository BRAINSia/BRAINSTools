...
class BRAINSResampleInputSpec(CommandLineInputSpec):
    ...
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="Resulting deformed image",
    ...
    pixelType = traits.Enum("float", "short", "ushort", "int", "uint", "uchar", "binary",
                            ...
                            argstr="--pixelType %s")
    deformationVolume = File(desc="Displacement Field to be used to warp the image (ITKv3 or earlier)", exists=True,
                             argstr="--deformationVolume %s")
    warpTransform = File(
        desc="Filename for the BRAINSFit transform (ITKv3 or earlier) or composite transform file (ITKv4)", exists=True,
    ...
    inverseTransform = traits.Bool(desc="True/False is to compute inverse of given transformation. Default is false",
