...
from nipype.interfaces.base import CommandLineInputSpec, SEMLikeCommandLine, TraitedSpec, File, Directory, traits, \
...
class BRAINSCutInputSpec(CommandLineInputSpec):
    netConfiguration = File(
        ...
        exists=True, argstr="--netConfiguration %s")
    modelConfigurationFilename = File(desc="XML File defining BRAINSCut parameters", exists=True,
    ...
    multiStructureThreshold = traits.Bool(desc="multiStructureThreshold module to deal with overlaping area",
    ...
    NoTrainingVectorShuffling = traits.Bool(desc="If this flag is on, there will be no shuffling.",
    ...
    modelFilename = traits.Str(desc=" model file name given from user (not by xml  configuration file) ",
...
class BRAINSROIAutoInputSpec(CommandLineInputSpec):
    inputVolume = File(desc="The input image for finding the largest region filled mask.", exists=True,
                       argstr="--inputVolume %s")
    outputROIMaskVolume = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    otsuPercentileThreshold = traits.Float(desc="Parameter to the Otsu threshold algorithm.",
    ...
    outputVolumePixelType = traits.Enum("float", "short", "ushort", "int", "uint", "uchar",
...
class BRAINSROIAutoOutputSpec(TraitedSpec):
    ...
    outputVolume = File(desc="The inputVolume with optional [maskOutput|cropOutput] to the region of the brain mask.",
                        exists=True)
...
class BRAINSConstellationDetectorInputSpec(CommandLineInputSpec):
    ...
    inputTemplateModel = File(desc="User-specified template model.,             ", exists=True,
    ...
    outputVolume = traits.Either(traits.Bool, File(
                                 ...
                                 argstr="--outputVolume %s")
    outputResampledVolume = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    outputVerificationScript = traits.Either(
        traits.Bool, File(), hash_files=False,
    ...
    outputUntransformedClippedVolume = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    rescaleIntensitiesOutputRange = InputMultiPath(
        ...
        sep=",", argstr="--rescaleIntensitiesOutputRange %s")
    ...
    interpolationMode = traits.Enum("NearestNeighbor", "Linear", "ResampleInPlace", "BSpline", "WindowedSinc",
    ...
    inputLandmarksEMSP = File(
        ...
        exists=True, argstr="--inputLandmarksEMSP %s")
    ...
    writeBranded2DImage = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    resultsDir = traits.Either(traits.Bool, Directory(), hash_files=False,
    ...
    numberOfThreads = traits.Int(desc="Explicitly specify the maximum number of threads to use.",
                                 argstr="--numberOfThreads %d")
    atlasVolume = File(desc="Atlas volume image to be used for BRAINSFit registration", exists=True,
                       argstr="--atlasVolume %s")
    atlasLandmarks = File(desc="Atlas landmarks to be used for BRAINSFit registration initialization,             ",
                          exists=True, argstr="--atlasLandmarks %s")
    atlasLandmarkWeights = File(
        ...
        exists=True, argstr="--atlasLandmarkWeights %s")
...
class BRAINSConstellationDetectorOutputSpec(TraitedSpec):
    ...
    writeBranded2DImage = File(
        desc=",               The filename for the 2D .png branded midline debugging image.  This will happen only in conjunction with requesting an outputVolume.,             ",
        exists=True)
    resultsDir = Directory(desc=",               The directory for the debuging images to be written.,             ",
                           exists=True)
...
"""
    ...
    _outputs_filenames = {'outputVolume': 'outputVolume.nii.gz', 'outputMRML': 'outputMRML.mrml',
                          'resultsDir': 'resultsDir', 'outputResampledVolume': 'outputResampledVolume.nii.gz',
                          'outputTransform': 'outputTransform.h5', 'writeBranded2DImage': 'writeBranded2DImage.png',
...
class BRAINSCreateLabelMapFromProbabilityMapsInputSpec(CommandLineInputSpec):
    inputProbabilityVolume = InputMultiPath(File(exists=True), desc="The list of proobabilityimages.",
                                            argstr="--inputProbabilityVolume %s...")
    priorLabelCodes = InputMultiPath(traits.Int,
                                     desc="A list of PriorLabelCode values used for coding the output label images",
                                     sep=",", argstr="--priorLabelCodes %s")
    foregroundPriors = InputMultiPath(traits.Int, desc="A list: For each Prior Label, 1 if foreground, 0 if background",
                                      sep=",", argstr="--foregroundPriors %s")
    nonAirRegionMask = File(desc="a mask representing the \'NonAirRegion\' -- Just force pixels in this region to zero",
                            exists=True, argstr="--nonAirRegionMask %s")
    inclusionThreshold = traits.Float(desc="tolerance for inclusion", argstr="--inclusionThreshold %f")
    dirtyLabelVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="the labels prior to cleaning",
                                     argstr="--dirtyLabelVolume %s")
    cleanLabelVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="the foreground labels volume",
...
class BinaryMaskEditorBasedOnLandmarksInputSpec(CommandLineInputSpec):
    inputBinaryVolume = File(desc="Input binary image in which to be edited", exists=True,
                             argstr="--inputBinaryVolume %s")
    outputBinaryVolume = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    inputLandmarksFilename = File(
        ...
        exists=True, argstr="--inputLandmarksFilename %s")
    inputLandmarkNames = InputMultiPath(traits.Str,
                                        desc=" A target input landmark name to be edited. This should be listed in the inputLandmakrFilename Given. ",
                                        sep=",", argstr="--inputLandmarkNames %s")
    setCutDirectionForLandmark = InputMultiPath(
        ...
        sep=",", argstr="--setCutDirectionForLandmark %s")
    setCutDirectionForObliquePlane = InputMultiPath(
        ...
        sep=",", argstr="--setCutDirectionForObliquePlane %s")
    inputLandmarkNamesForObliquePlane = InputMultiPath(traits.Str,
                                                       ...
                                                       sep=",", argstr="--inputLandmarkNamesForObliquePlane %s")
...
class BRAINSMultiSTAPLEInputSpec(CommandLineInputSpec):
    inputCompositeT1Volume = File(desc="Composite T1, all label maps transofrmed into the space for this image.",
                                  exists=True, argstr="--inputCompositeT1Volume %s")
    inputLabelVolume = InputMultiPath(File(exists=True), desc="The list of proobabilityimages.",
                                      argstr="--inputLabelVolume %s...")
    inputTransform = InputMultiPath(File(exists=True), desc="transforms to apply to label volumes",
    ...
    resampledVolumePrefix = traits.Str(desc="if given, write out resampled volumes with this prefix",
    ...
    outputMultiSTAPLE = traits.Either(traits.Bool, File(), hash_files=False,
                                      desc="the MultiSTAPLE average of input label volumes",
                                      argstr="--outputMultiSTAPLE %s")
    outputConfusionMatrix = traits.Either(traits.Bool, File(), hash_files=False, desc="Confusion Matrix",
...
"""
    ...
    _outputs_filenames = {'outputMultiSTAPLE': 'outputMultiSTAPLE.nii',
                          'outputConfusionMatrix': 'outputConfusionMatrixh5|mat|txt'}
    _redirect_x = False


class BRAINSABCInputSpec(CommandLineInputSpec):
    inputVolumes = InputMultiPath(File(exists=True), desc="The list of input image files to be segmented.",
    ...
    inputVolumeTypes = InputMultiPath(traits.Str,
                                      desc="The list of input image types corresponding to the inputVolumes.", sep=",",
                                      argstr="--inputVolumeTypes %s")
    outputDir = traits.Either(traits.Bool, Directory(), hash_files=False, desc="Ouput directory",
                              argstr="--outputDir %s")
    atlasToSubjectTransformType = traits.Enum("Identity", "Rigid", "Affine", "BSpline", "SyN",
    ...
    atlasToSubjectTransform = traits.Either(traits.Bool, File(), hash_files=False,
                                            desc="The transform from atlas to the subject",
                                            argstr="--atlasToSubjectTransform %s")
    atlasToSubjectInitialTransform = traits.Either(traits.Bool, File(), hash_files=False,
                                                   desc="The initial transform from atlas to the subject",
                                                   argstr="--atlasToSubjectInitialTransform %s")
    subjectIntermodeTransformType = traits.Enum("Identity", "Rigid", "Affine", "BSpline",
                                                desc=" What type of linear transform type do you want to use to register the atlas to the reference subject image.",
    ...
    outputLabels = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Label Image",
                                 argstr="--outputLabels %s")
    outputDirtyLabels = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Dirty Label Image",
    ...
    interpolationMode = traits.Enum("BSpline", "NearestNeighbor", "WindowedSinc", "Linear", "ResampleInPlace",
                                    "Hamming", "Cosine", "Welch", "Lanczos", "Blackman",
    ...
    filterMethod = traits.Enum("None", "CurvatureFlow", "GradientAnisotropicDiffusion", "Median",
                               desc="Filter method for preprocessing of registration", argstr="--filterMethod %s")
    ...
    gridSize = InputMultiPath(traits.Int, desc="Grid size for atlas warping with BSplines", sep=",",
                              argstr="--gridSize %s")
    defaultSuffix = traits.Str(argstr="--defaultSuffix %s")
    implicitOutputs = traits.Either(traits.Bool, InputMultiPath(File(), ), hash_files=False,
...
class BRAINSABCOutputSpec(TraitedSpec):
    ...
    implicitOutputs = OutputMultiPath(File(exists=True),
                                      desc="Outputs to be made available to NiPype. Needed because not all BRAINSABC outputs have command line arguments.",
                                      exists=True)
...
"""
    ...
    _cmd = " BRAINSABC "
    _outputs_filenames = {'outputLabels': 'outputLabels.nii.gz',
                          'atlasToSubjectTransform': 'atlasToSubjectTransform.h5',
                          'atlasToSubjectInitialTransform': 'atlasToSubjectInitialTransform.h5',
                          'outputDirtyLabels': 'outputDirtyLabels.nii.gz', 'outputVolumes': 'outputVolumes.nii.gz',
                          'outputDir': 'outputDir', 'implicitOutputs': 'implicitOutputs.nii.gz'}
    _redirect_x = False


class ESLRInputSpec(CommandLineInputSpec):
    inputVolume = File(desc="Input Label Volume", exists=True, argstr="--inputVolume %s")
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Label Volume",
                                 argstr="--outputVolume %s")
    ...
    preserveOutside = traits.Bool(desc="For values outside the specified range, preserve those values.",
