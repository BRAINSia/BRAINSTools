...
class BRAINSConstellationModelerInputSpec(CommandLineInputSpec):
    ...
    inputTrainingList = File(
        ...
        exists=True, argstr="--inputTrainingList %s")
    outputModel = traits.Either(traits.Bool, File(), hash_files=False,
...
class BRAINSConstellationModelerOutputSpec(TraitedSpec):
    ...
    resultsDir = Directory(desc=",               The directory for the results to be written.,             ",
                           exists=True)
...
class landmarksConstellationWeightsInputSpec(CommandLineInputSpec):
    ...
    outputWeightsList = traits.Either(traits.Bool, File(), hash_files=False,
...
class BRAINSTrimForegroundInDirectionInputSpec(CommandLineInputSpec):
    inputVolume = File(desc="Input image to trim off the neck (and also air-filling noise.)", exists=True,
...
class BRAINSLmkTransformInputSpec(CommandLineInputSpec):
    inputMovingLandmarks = File(desc="Input Moving Landmark list file in fcsv,             ", exists=True,
                                argstr="--inputMovingLandmarks %s")
    inputFixedLandmarks = File(desc="Input Fixed Landmark list file in fcsv,             ", exists=True,
                               argstr="--inputFixedLandmarks %s")
    outputAffineTransform = traits.Either(traits.Bool, File(), hash_files=False,
    ...
    inputReferenceVolume = File(desc="The filename of the reference volume", exists=True,
...
"""
    ...
    _outputs_filenames = {'outputResampledVolume': 'outputResampledVolume.nii',
                          'outputAffineTransform': 'outputAffineTransform.h5'}
    _redirect_x = False


class BRAINSMushInputSpec(CommandLineInputSpec):
    inputFirstVolume = File(desc="Input image (1) for mixture optimization", exists=True,
                            argstr="--inputFirstVolume %s")
    inputSecondVolume = File(desc="Input image (2) for mixture optimization", exists=True,
                             argstr="--inputSecondVolume %s")
    inputMaskVolume = File(desc="Input label image for mixture optimization", exists=True,
                           argstr="--inputMaskVolume %s")
    outputWeightsFile = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Weights File",
                                      argstr="--outputWeightsFile %s")
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False,
                                 desc="The MUSH image produced from the T1 and T2 weighted images",
                                 argstr="--outputVolume %s")
    outputMask = traits.Either(traits.Bool, File(), hash_files=False,
                               desc="The brain volume mask generated from the MUSH image", argstr="--outputMask %s")
    seed = InputMultiPath(traits.Int, desc="Seed Point for Brain Region Filling", sep=",", argstr="--seed %s")
    desiredMean = traits.Float(desc="Desired mean within the mask for weighted sum of both images.",
                               argstr="--desiredMean %f")
    desiredVariance = traits.Float(desc="Desired variance within the mask for weighted sum of both images.",
                                   argstr="--desiredVariance %f")
    lowerThresholdFactorPre = traits.Float(desc="Lower threshold factor for finding an initial brain mask",
                                           argstr="--lowerThresholdFactorPre %f")
    upperThresholdFactorPre = traits.Float(desc="Upper threshold factor for finding an initial brain mask",
                                           argstr="--upperThresholdFactorPre %f")
    lowerThresholdFactor = traits.Float(desc="Lower threshold factor for defining the brain mask",
                                        argstr="--lowerThresholdFactor %f")
    upperThresholdFactor = traits.Float(desc="Upper threshold factor for defining the brain mask",
                                        argstr="--upperThresholdFactor %f")
    boundingBoxSize = InputMultiPath(traits.Int,
                                     desc="Size of the cubic bounding box mask used when no brain mask is present",
                                     sep=",", argstr="--boundingBoxSize %s")
    boundingBoxStart = InputMultiPath(traits.Int,
                                      desc="XYZ point-coordinate for the start of the cubic bounding box mask used when no brain mask is present",
                                      sep=",", argstr="--boundingBoxStart %s")
    numberOfThreads = traits.Int(desc="Explicitly specify the maximum number of threads to use.",
                                 argstr="--numberOfThreads %d")
...
"""
    ...
    _cmd = " BRAINSMush "
    _outputs_filenames = {'outputMask': 'outputMask.nii.gz', 'outputWeightsFile': 'outputWeightsFile.txt',
...
class BRAINSTransformConvertInputSpec(CommandLineInputSpec):
    ...
    outputTransformType = traits.Enum("Affine", "VersorRigid", "ScaleVersor", "ScaleSkewVersor", "DisplacementField",
                                      "Same",
    ...
    outputPrecisionType = traits.Enum("double", "float",
...
class landmarksConstellationAlignerInputSpec(CommandLineInputSpec):
    ...
    outputLandmarksPaired = traits.Either(traits.Bool, File(), hash_files=False, desc="Output landmark file (.fcsv)",
...
class BRAINSEyeDetectorInputSpec(CommandLineInputSpec):
    ...
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="The output volume",
                                 argstr="--outputVolume %s")
...
class BRAINSLinearModelerEPCAInputSpec(CommandLineInputSpec):
    inputTrainingList = File(desc="Input Training Landmark List Filename,             ", exists=True,
...
class BRAINSInitializedControlPointsInputSpec(CommandLineInputSpec):
    ...
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False, desc="Output Volume",
                                 argstr="--outputVolume %s")
    splineGridSize = InputMultiPath(traits.Int,
                                    ...
                                    sep=",", argstr="--splineGridSize %s")
    permuteOrder = InputMultiPath(traits.Int,
                                  desc="The permutation order for the images.  The default is 0,1,2 (i.e. no permutation)",
                                  sep=",", argstr="--permuteOrder %s")
...
class CleanUpOverlapLabelsInputSpec(CommandLineInputSpec):
    ...
    outputBinaryVolumes = traits.Either(traits.Bool, InputMultiPath(File(), ), hash_files=False,
...
class CleanUpOverlapLabelsOutputSpec(TraitedSpec):
    outputBinaryVolumes = OutputMultiPath(File(exists=True),
                                          desc="The output label map images, with integer values in it. Each label value specified in the inputLabels is combined into this output label map volume",
                                          exists=True)
...
class BRAINSClipInferiorInputSpec(CommandLineInputSpec):
    inputVolume = File(desc="Input image to make a clipped short int copy from.", exists=True,
                       argstr="--inputVolume %s")
    outputVolume = traits.Either(traits.Bool, File(), hash_files=False,
...
class GenerateLabelMapFromProbabilityMapInputSpec(CommandLineInputSpec):
    inputVolumes = InputMultiPath(File(exists=True), desc="The Input probaiblity images to be computed for lable maps",
                                  argstr="--inputVolumes %s...")
    outputLabelVolume = traits.Either(traits.Bool, File(), hash_files=False,
...
class BRAINSAlignMSPInputSpec(CommandLineInputSpec):
    ...
    OutputresampleMSP = traits.Either(traits.Bool, File(), hash_files=False,
                                      desc=",         The image to be output.,       ", argstr="--OutputresampleMSP %s")
    verbose = traits.Bool(desc=",         Show more verbose output,       ", argstr="--verbose ")
    resultsDir = traits.Either(traits.Bool, Directory(), hash_files=False,
                               desc=",         The directory for the results to be written.,       ",
                               argstr="--resultsDir %s")
    ...
    mspQualityLevel = traits.Int(
        desc=",           Flag cotrols how agressive the MSP is estimated.  0=quick estimate (9 seconds), 1=normal estimate (11 seconds), 2=great estimate (22 seconds), 3=best estimate (58 seconds).,       ",
        argstr="--mspQualityLevel %d")
    rescaleIntensities = traits.Bool(desc=",           Flag to turn on rescaling image intensities on input.,       ",
                                     argstr="--rescaleIntensities ")
    trimRescaledIntensities = traits.Float(
        desc=",           Turn on clipping the rescaled image one-tailed on input.  Units of standard deviations above the mean.  Very large values are very permissive.  Non-positive value turns clipping off.  Defaults to removing 0.00001 of a normal tail above the mean.,       ",
        argstr="--trimRescaledIntensities %f")
    rescaleIntensitiesOutputRange = InputMultiPath(traits.Int,
                                                   desc=",           This pair of integers gives the lower and upper bounds on the signal portion of the output image.  Out-of-field voxels are taken from BackgroundFillValue.,       ",
                                                   sep=",", argstr="--rescaleIntensitiesOutputRange %s")
    BackgroundFillValue = traits.Str(
        desc="Fill the background of image with specified short int value. Enter number or use BIGNEG for a large negative number.",
...
class BRAINSLandmarkInitializerInputSpec(CommandLineInputSpec):
    inputFixedLandmarkFilename = File(desc="input fixed landmark. *.fcsv", exists=True,
                                      argstr="--inputFixedLandmarkFilename %s")
    inputMovingLandmarkFilename = File(desc="input moving landmark. *.fcsv", exists=True,
    ...
    inputWeightFilename = File(
        ...
        exists=True, argstr="--inputWeightFilename %s")
    outputTransformFilename = traits.Either(traits.Bool, File(), hash_files=False,
...
class insertMidACPCpointInputSpec(CommandLineInputSpec):
    ...
    outputLandmarkFile = traits.Either(traits.Bool, File(), hash_files=False, desc="Output landmark file (.fcsv)",
...
class BRAINSSnapShotWriterInputSpec(CommandLineInputSpec):
    ...
    inputSliceToExtractInPhysicalPoint = InputMultiPath(traits.Float,
                                                        ...
                                                        sep=",", argstr="--inputSliceToExtractInPhysicalPoint %s")
    inputSliceToExtractInIndex = InputMultiPath(traits.Int,
                                                ...
                                                sep=",", argstr="--inputSliceToExtractInIndex %s")
    inputSliceToExtractInPercent = InputMultiPath(traits.Int,
                                                  ...
                                                  sep=",", argstr="--inputSliceToExtractInPercent %s")
    inputPlaneDirection = InputMultiPath(traits.Int,
                                         desc="Plane to display. In general, 0=saggital, 1=coronal, and 2=axial plane.",
                                         sep=",", argstr="--inputPlaneDirection %s")
    outputFilename = traits.Either(traits.Bool, File(), hash_files=False,
                                   desc="2D file name of input images. Required.", argstr="--outputFilename %s")
...
class JointHistogramInputSpec(CommandLineInputSpec):
    inputVolumeInXAxis = File(desc="The Input image to be computed for statistics", exists=True,
                              argstr="--inputVolumeInXAxis %s")
    inputVolumeInYAxis = File(desc="The Input image to be computed for statistics", exists=True,
    ...
    inputMaskVolumeInXAxis = File(
        ...
        exists=True, argstr="--inputMaskVolumeInXAxis %s")
    inputMaskVolumeInYAxis = File(
        ...
        exists=True, argstr="--inputMaskVolumeInYAxis %s")
...
class ShuffleVectorsModuleInputSpec(CommandLineInputSpec):
    inputVectorFileBaseName = File(
        ...
        exists=True, argstr="--inputVectorFileBaseName %s")
    outputVectorFileBaseName = traits.Either(traits.Bool, File(), hash_files=False,
...
class ImageRegionPlotterInputSpec(CommandLineInputSpec):
    ...
    inputBinaryROIVolume = File(desc="The Input binary image for region of interest", exists=True,
    ...
    useROIAUTO = traits.Bool(
        ...
        argstr="--useROIAUTO ")
    useIntensityForHistogram = traits.Bool(desc=" Create Intensity Joint Histogram instead of Quantile Joint Histogram",
