...
def CreateANTSRegistrationWorkflow(WFname,CLUSTER_QUEUE,NumberOfThreads=-1):
    ...
    BF_cpu_sge_options_dictionary={'qsub_args': '-S /bin/bash -pe smp1 2-4 -l h_vmem=4G,mem_free=4G -o /dev/null -e /dev/null '+CLUSTER_QUEUE, 'overwrite': True}
   ...
   # ANTSWF.connect(inputsSpec,'movingBinaryVolume',BFitAtlasToSubject,'movingBinaryVolume')
    ...
    many_cpu_sge_options_dictionary={'qsub_args': '-S /bin/bash -pe smp1 4 -l h_vmem=7G,mem_free=7G -o /dev/null -e /dev/null '+CLUSTER_QUEUE, 'overwrite': True}
