...
def CreateANTSRegistrationWorkflow(WFname, CLUSTER_QUEUE, CLUSTER_QUEUE_LONG, NumberOfThreads=-1):
    ...
    BF_cpu_sge_options_dictionary = {'qsub_args': '-S /bin/bash -pe smp1 2-64 -l h_vmem=14G,mem_free=4G -o /dev/null -e /dev/null ' + CLUSTER_QUEUE, 'overwrite': True}
   ...
   # ANTSWF.connect(inputsSpec,'movingBinaryVolume',BFitAtlasToSubject,'movingBinaryVolume')
    ...
    many_cpu_sge_options_dictionary = {'qsub_args': '-S /bin/bash -pe smp1 5-64 -l h_vmem=17G,mem_free=9G -o /dev/null -e /dev/null ' + CLUSTER_QUEUE, 'overwrite': True}
