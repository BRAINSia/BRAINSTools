...
def create_autorecon1(config):
    ...
    if not config["longitudinal"]:
        ...
        if config["in_T2"] != None:
            ...
            T2_convert.inputs.out_file = os.path.join(
                config["subjects_dir"], config["current_id"], "mri", "orig", "T2raw.mgz"
            )
        ...
        if config["in_FLAIR"] != None:
            ...
            FLAIR_convert.inputs.out_file = os.path.join(
                config["subjects_dir"],
                config["current_id"],
                "mri",
                "orig",
        ...
        _volnames_, in_iscales, in_ltas = create_preproc_filenames(
            config["subjects_dir"], config["current_id"], config["in_T1s"]
        )
        ...
        ar1_wf.connect(
            [
                (copy_ltas, concatenate_lta, [("out_file", "in_file")]),
                (
                    inputSpec,
                    concatenate_lta,
                    [("subj_to_template_lta", "subj_to_base")],
                ),
            ]
        )
    ...
    if not config["longitudinal"] and len(config["in_T1s"]) == 1:
        ...
        ar1_wf.connect(
            [(T1_image_preparation, create_template, [("out_file", "in_file")])]
        )
        print(
            """
...
copy the run to rawavg and continue."""
        ...
        create_template.inputs.out_file = os.path.join(
            config["subjects_dir"], config["current_id"], "mri", "rawavg.mgz"
        )
        ...
        if config["longitudinal"]:
            ...
            ar1_wf.connect(
                [(T1_image_preparation, create_template, [("out_file", "in_files")])]
            )

    # mri_convert
    conform_template = pe.Node(MRIConvert(), name="Conform_Template")
    conform_template.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "orig.mgz"
    )
    if config["longitudinal"]:
    ...
    bias_correction.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "orig_nu.mgz"
    )

    ar1_wf.connect([(add_to_header, bias_correction, [("out_file", "in_file")])])

    if config["longitudinal"]:
        copy_template_xfm = pe.Node(
            Function(["in_file", "out_file"], ["out_file"], copy_file),
    ...
    if not config["longitudinal"]:
        awk_logfile = pe.Node(
            Function(["awk_file", "log_file"], ["log_file"], awk), name="Awk"
        )
        awk_logfile.inputs.awk_file = os.path.join(
            config["FREESURFER_HOME"], "bin", "extract_talairach_avi_QA.awk"
        )
    ...
    mri_normalize.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "T1.mgz"
    )
    ar1_wf.connect(
        [
            (bias_correction, mri_normalize, [("out_file", "in_file")]),
            (copy_transform, mri_normalize, [("out_file", "transform")]),
        ]
    )
    ...
    if config["longitudinal"]:
        ...
        mask1.inputs.out_file = os.path.join(
            config["subjects_dir"], config["current_id"], "mri", "brainmask.auto.mgz"
        )

        ar1_wf.connect(
            [
                (mri_normalize, mask1, [("out_file", "in_file")]),
        ...
        ar1_wf.connect(
            [
                (mask1, mask2, [("out_file", "in_file"), ("out_file", "out_file")]),
        ...
        mri_em_register.inputs.template = os.path.join(
            config["FREESURFER_HOME"], "average", "RB_all_withskull_2014-08-21.gca"
        )
        mri_em_register.inputs.out_file = os.path.join(
            config["subjects_dir"],
            config["current_id"],
            "mri",
            "transforms",
        ...
        watershed_skull_strip.inputs.brain_atlas = os.path.join(
            config["FREESURFER_HOME"], "average", "RB_all_withskull_2014-08-21.gca"
        )
        watershed_skull_strip.inputs.out_file = os.path.join(
            config["subjects_dir"], config["current_id"], "mri", "brainmask.auto.mgz"
        )
        ar1_wf.connect(
            [
                (mri_normalize, watershed_skull_strip, [("out_file", "in_file")]),
                (mri_em_register, watershed_skull_strip, [("out_file", "transform")]),
            ]
        )
    ...
    copy_brainmask.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "brainmask.mgz"
    )
    ...
    if config["longitudinal"]:
        ...
        ar1_wf.connect(
            [(watershed_skull_strip, copy_brainmask, [("brain_vol", "in_file")])]
        )
