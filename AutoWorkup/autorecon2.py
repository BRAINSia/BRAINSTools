...
def create_autorecon2(config):
    ...
    if config["longitudinal"]:
        ...
        inputSpec = pe.Node(
            IdentityInterface(
                fields=["orig", "brainmask", "transform", "subject_id", "subjects_dir"]
            ),
            run_without_submitting=True,
            name="AutoRecon2_Inputs",
        )
    ...
    """
    intensity_correction = pe.Node(MNIBiasCorrection(), name="Intensity_Correction")
    intensity_correction.inputs.iterations = 1
    intensity_correction.inputs.protocol_iterations = 1000
    intensity_correction.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "nu.mgz"
    )
    ar2_wf.connect(
        [
            (
    ...
    ar2_wf.connect(
        [
            (
                intensity_correction,
                add_to_header_nu,
                [("out_file", "in_file"), ("out_file", "out_file")],
            ),
    ...
    if config["longitudinal"]:
        ...
        align_transform.inputs.out_file = os.path.join(
            config["subjects_dir"],
            config["current_id"],
            "mri",
            "transforms",
        ...
        ar2_wf.connect(
            [(inputSpec, align_transform, [("template_talairach_lta", "in_file")])]
        )

    else:
        align_transform = pe.Node(EMRegister(), name="Align_Transform")
        align_transform.inputs.template = os.path.join(
        ...
        ar2_wf.connect(
            [
                (inputSpec, align_transform, [("brainmask", "mask")]),
    ...
    ca_normalize.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "norm.mgz"
    )
    ca_normalize.inputs.atlas = os.path.join(
        config["FREESURFER_HOME"], "average", "RB_all_2014-08-21.gca"
    )
    if config["longitudinal"]:
        ...
        ca_normalize.inputs.control_points = os.path.join(
            config["subjects_dir"], config["current_id"], "mri", "ctrl_pts.mgz"
        )

    ar2_wf.connect(
        [
            (align_transform, ca_normalize, [("out_file", "transform")]),
    ...
    ca_register.inputs.template = os.path.join(
        config["FREESURFER_HOME"], "average", "RB_all_2014-08-21.gca"
    )
    ca_register.inputs.out_file = os.path.join(
        config["subjects_dir"],
        config["current_id"],
        "mri",
        "transforms",
    ...
    ar2_wf.connect(
        [
            (ca_normalize, ca_register, [("out_file", "in_file")]),
    ...
    if config["longitudinal"]:
        ...
        ar2_wf.connect(
            [(ar1_inputs, ca_register, [("template_talairach_m3z", "l_files")])]
        )
    else:
    ...
    remove_neck.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "nu_noneck.mgz"
    )
    remove_neck.inputs.template = os.path.join(
        config["FREESURFER_HOME"], "average", "RB_all_2014-08-21.gca"
    )
    ar2_wf.connect(
        [
            (ca_register, remove_neck, [("out_file", "transform")]),
    ...
    em_reg_withskull.inputs.out_file = os.path.join(
        config["subjects_dir"],
        config["current_id"],
        "mri",
        "transforms",
    ...
    ar2_wf.connect(
        [
            (align_transform, em_reg_withskull, [("out_file", "transform")]),
    ...
    if config["longitudinal"]:
        ...
        ar2_wf.connect(
            [
                (inputSpec, merge_norms, [("alltps_norms", "in1")]),
        ...
        fuse_segmentations.inputs.out_file = os.path.join(
            config["subjects_dir"], config["current_id"], "mri", "aseg.fused.mgz"
        )
    ...
    ca_label.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "aseg.auto_noCCseg.mgz"
    )
    ...
    ar2_wf.connect(
        [
            (ca_normalize, ca_label, [("out_file", "in_file")]),
    ...
    if config["longitudinal"]:
        ar2_wf.connect(
            [
                (fuse_segmentations, ca_label, [("out_file", "in_vol")]),
    ...
    segment_cc.inputs.out_rotation = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "transforms", "cc_up.lta"
    )
    segment_cc.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "aseg.auto.mgz"
    )
    ar2_wf.connect(
        [
            (
    ...
    copy_cc.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "aseg.presurf.mgz"
    )
    ...
    normalization2.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "brain.mgz"
    )
    ar2_wf.connect(
        [
            (copy_cc, normalization2, [("out_file", "segmentation")]),
    ...
    mri_mask.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "brain.finalsurfs.mgz"
    )

    ar2_wf.connect(
        [
            (normalization2, mri_mask, [("out_file", "in_file")]),
    ...
    wm_seg.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "wm.seg.mgz"
    )
    ar2_wf.connect([(normalization2, wm_seg, [("out_file", "in_file")])])

    edit_wm = pe.Node(EditWMwithAseg(), name="Edit_WhiteMatter")
    edit_wm.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "wm.asegedit.mgz"
    )
    edit_wm.inputs.keep_in = True
    ar2_wf.connect(
        [
            (wm_seg, edit_wm, [("out_file", "in_file")]),
    ...
    pretess.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "wm.mgz"
    )
    pretess.inputs.label = "wm"
    ar2_wf.connect(
        [
            (edit_wm, pretess, [("out_file", "in_filled")]),
        ...
        ar2_wf.connect(
            [
                ...
                (
                    inputSpec,
                    transfer_init_wm,
                    [("init_wm", "mask_file"), ("subj_to_template_lta", "xfm_file")],
                ),
            ]
        )
    ...
    fill.inputs.log_file = os.path.join(
        config["subjects_dir"], config["current_id"], "scripts", "ponscc.cut.log"
    )
    fill.inputs.out_file = os.path.join(
        config["subjects_dir"], config["current_id"], "mri", "filled.mgz"
    )
    ar2_wf.connect(
        [
    ...
    for hemisphere in ["lh", "rh"]:
        ...
        if config["longitudinal"]:
            ...
            copy_template_white.inputs.out_file = os.path.join(
                config["subjects_dir"],
                config["current_id"],
                "surf",
            ...
            copy_template_orig_white.inputs.out_file = os.path.join(
                config["subjects_dir"],
                config["current_id"],
                "surf",
            ...
            copy_template_orig_pial.inputs.out_file = os.path.join(
                config["subjects_dir"],
                config["current_id"],
                "surf",
            ...
            hemi_wf.connect(
                [
                    (
                        ...
                        [("surface", "in_file"), ("surface", "out_file")],
                    )
                ]
            )
            ...
            hemi_wf.connect(
                [(extract_main_component, copy_orig, [("out_file", "in_file")])]
            )
            ...
            hemi_wf.connect(
                [(extract_main_component, smooth1, [("out_file", "in_file")])]
            )
            ...
            hemi_wf.connect(
                [
                    (copy_orig, fix_topology, [("out_file", "in_orig")]),
            ...
            remove_inflate1 = pe.Node(
                Function(["in_file", "dependent"], ["out_file"], rmfile),
                name="Remove_Inflate1",
            )

            hemi_wf.connect(
                [
                    (copy_inflate1, remove_inflate1, [("out_file", "in_file")]),
                    (remove_intersection, remove_inflate1, [("out_file", "dependent")]),
                ]
            )
            ...
            make_surfaces.inputs.hemisphere = hemisphere
            hemi_wf.connect(
                [(remove_intersection, make_surfaces, [("out_file", "in_orig")])]
            )
        ...
        curvature2.inputs.threshold = 0.999
        ...
        ar2_wf.connect(
            [
                (ca_normalize, hemi_wf, [("out_file", "Pretess2.in_norm")]),
                (fill, hemi_wf, [("out_file", "Pretess2.in_filled")]),
                (
                    inputSpec,
                    hemi_wf,
                    [
                        ("subject_id", "Fix_Topology.subject_id"),
                ...
                (
                    ...
                    hemi_wf,
                    [
                        ("subject_id", "Make_Surfaces.subject_id"),
                ...
                (
                    ...
                    hemi_wf,
                    [
                        ("subject_id", "Curvature_Stats.subject_id"),
