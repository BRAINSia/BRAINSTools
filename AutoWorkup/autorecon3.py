...
def create_autorecon3(config):
    ...
    ar3_lh_wf.connect(
        [
            (ar3_sphere, ar3_surfreg, [("out_file", "in_surf")]),
            (
                ar3_lh_inputs,
                ar3_surfreg,
                [
    ...
    ar3_lh_wf.connect(
        [
            (ar3_lh_inputs, ar3_jacobian, [("white", "in_origsurf")]),
    ...
    ar3_lh_wf.connect(
        [
            (ar3_surfreg, ar3_paint, [("out_file", "in_surf")]),
    ...
    # Workflow1 Outputs
    outputs1 = pe.Node(IdentityInterface(fields=["pial"]), name="Outputs")
    ...
    ar3_wf.connect(
        [
            ...
            (ar3_lh_wf, volume_mask, [("Make_Pial_Surface.out_pial", "lh_pial")]),
            (ar3_rh_wf, volume_mask, [("Make_Pial_Surface.out_pial", "rh_pial")]),
        ]
    )
    ...
    for hemisphere, workflow in [("lh", ar3_lh_wf), ("rh", ar3_rh_wf)]:
        ...
        parcellation_stats_white.inputs.out_color = os.path.join(
            config["subjects_dir"], config["current_id"], "label", "aparc.annot.ctab"
        )
        ...
        [
            ...
            (
                ...
                [
                    ...
                    ("Cortical_Parcellation.out_file", "lh_annotation"),
                ],
            ),
            (
                ar3_rh_wf,
                ...
                [
                    ...
                    ("Cortical_Parcellation.out_file", "rh_annotation"),
                ],
            ),
            (
                volume_mask,
    ...
    ar3_wf.connect(
        [
            ...
            (lh_cort_parc2, aparc_2_aseg_2009, [("out_file", "lh_annotation")]),
            (rh_cort_parc2, aparc_2_aseg_2009, [("out_file", "rh_annotation")]),
    ...
    ar3_wf.connect(
        [
            (
                ...
                ba_WF,
                [
                    ("Surface_Registration.out_file", "Inputs.lh_sphere_reg"),
                    ("Make_Pial_Surface.out_thickness", "Inputs.lh_thickness"),
                    ("Make_Pial_Surface.out_pial", "Inputs.lh_pial"),
                ],
            ),
            (
                ar3_rh_wf,
                ba_WF,
                [
                    ("Surface_Registration.out_file", "Inputs.rh_sphere_reg"),
                    ("Make_Pial_Surface.out_thickness", "Inputs.rh_thickness"),
                    ("Make_Pial_Surface.out_pial", "Inputs.rh_pial"),
                ],
            ),
            (
                inputSpec,
            ...
            for connection, meas_file, meas_name in [
                ...
                if not os.path.isdir(target_dir):
                    ...
                    target_home = os.path.join(
                        config["FREESURFER_HOME"], "subjects", target_id
                    )
                    # Create a symlink
                ...
                ar3_wf.connect(
                    [
                        (
                            hemi_wf,
                            preprocess,
                            [("Surface_Registration.out_file", "surfreg_file")],
                        ),
                ...
                for value in range(0, 26, 5):
                    ...
                    surf2surf.inputs.out_file = os.path.join(
                        config["subjects_dir"], config["current_id"], "surf", tval_file
                    )
                    ar3_lh_wf.connect(
                        [
                            (preprocess, surf2surf, [("out_file", "in_file")]),
                            (inputSpec, surf2surf, [("subjects_dir", "subjects_dir")]),
                        ]
                    )
