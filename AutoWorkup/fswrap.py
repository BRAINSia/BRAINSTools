...
class FSScriptInputSpec(CommandLineInputSpec):
    T1_files = File(
        argstr='--T1image %s', exists=True, desc='Original T1 image')
    ...
    subjects_dir = Directory(
        argstr='--FSSubjDir %s', desc='FreeSurfer subjects directory')
...
# TODO:                         desc='Location of FreeSurfer (differs for
...
class FSScript(CommandLine):
    ...
    _cmd = '{0} {1}'.format(
        sys.executable, os.path.join(this_path, 'fsscript.py'))
# _cmd = 'fsscript.py'
    ...
    def _list_outputs(self):
        ...
        outputs['label1_out'] = os.path.join(
            os.getcwd(), 'mri_nifti', 'aparc+aseg.nii.gz')
        outputs['label2_out'] = os.path.join(
            os.getcwd(), 'mri_nifti', 'aparc.a2009+aseg.nii.gz')
        return outputs
