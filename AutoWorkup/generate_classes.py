...
def generate_all_classes(modules_list = [], launcher=[], exec_dir='', outdir='./'):
    ...
    """
    outdir=outdir.strip('/')+'/' ## ensure that outdir has exactly 1 trailing '/'
    init_imports = ""
    for module in modules_list:
        ...
        f = open(outdir+module_python_filename, "w")
        f.write(code)
        f.close()
        init_imports += "from %s import %s\n"%(module,module)
    f = open(outdir+"__init__.py", "w")
