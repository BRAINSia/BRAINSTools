...
def collect_training_data(training_files):
    ...
    for t1_file, additional_files, truth_files in training_files:
        ...
        gm_truth_data = pd.Series(
            linear_array_from_image_file(truth_files["gm"]), name="GM", index=index
        )
        wm_truth_data = pd.Series(
            linear_array_from_image_file(truth_files["wm"]), name="WM", index=index
        )
