...
def get_edges(nm_file, between_labels=False):
    ...
    wm_mask = fill_mask_holes(
        sitk.BinaryThreshold(nm_map, 0.5, 1000) * (gm_mask == 0) * (background == 0)
    )
    ...
    if between_labels:
        ...
        gm_map = sitk.Cast(nm_map, sitk.sitkUInt16) * sitk.Cast(
            gm_mask, sitk.sitkUInt16
        )
        ...
        gm_contours = (sitk.LabelContour(sitk.Or(gm_mask, wm_mask)) > 0) * (
            wm_contours == 0
        )
...
def create_water_sheds(aseg_file, filled_file, dilation=6):
    ...
    # 53 == rh hippocamus
    # 54 == rh amygdala
    rh_hipthal = ((fs == 49) + (fs == 53) + (fs == 54)) * wm_labels[
        1
    ]  # assigns label 11
    lh_hipthal = ((fs == 10) + (fs == 17) + (fs == 18)) * wm_labels[
        1
    ]  # assigns label 11
...
def create_data_csv(in_dir, cache_dir, overwrite=False, file_list=None):
    ...
    for i, nm_file in enumerate(file_list):
        ...
        out_fs_file = convert_fs_scan(
            in_fs_file,
            os.path.join(cache_dir, subject_id, "aseg.nii.gz"),
            resample_type="nearest",
        )
        in_filled_file = get_nm_fs_scan(nm_file, name="filled.mgz")
        out_filled_file = convert_fs_scan(
            in_filled_file,
        ...
        if not (wlf and wtf and glf and gtf) or overwrite:
            # define truth images
            nm_converted_file = convert_fs_scan(
                nm_file,
