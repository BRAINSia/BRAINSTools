...
from nipype.interfaces.base import BaseInterface, traits, TraitedSpec, DynamicTraitedSpec, File, BaseInterfaceInputSpec, \
    isdefined
...
def writeCVSubsetFile(environment, experiment, pipeline, cluster, csv_file, test_size, hasHeader):
    ...
    for subset in cv_subsets:
        ...
        for testSession in testData:
            ...
            CV_JointFusion_WF.connect(testSessionSpec, 'template_leftHemisphere', myJointFusion,
                                      'inputspec.subj_left_hemisphere')
            CV_JointFusion_WF.connect(testSessionSpec, 'landmarkInACPCAlignedSpace', myJointFusion,
                                      'inputspec.subj_lmks')
            CV_JointFusion_WF.connect(testSessionSpec, 'template_weights_50Lmks_wts', myJointFusion,
