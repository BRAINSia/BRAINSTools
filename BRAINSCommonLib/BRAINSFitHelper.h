...
BRAINSFitHelper::SetupRegistration( GenericMetricType * costMetric )
{
  ...
  if ( this->m_FixedBinaryVolume.IsNotNull() )
  {
    ...
    itk::ImageRandomNonRepeatingConstIteratorWithIndex< FixedImageType > NRit(
      this->m_FixedVolume, this->m_FixedVolume->GetBufferedRegion() );
  ...
  myHelper->SetInitializeRegistrationByCurrentGenericTransform(
    this->m_InitializeRegistrationByCurrentGenericTransform );
