...
#define VTROExceptionMacroMacro()                                                                                      \
  ...
  for ( unsigned int l = 0; l < transformType.size(); ++l )
  {
    ...
    {
      std::cerr << " ERROR:  Invalid transform type specified for element " << l
                << " of --transformType: " << transformType[l] << std::endl;
      VTROExceptionMacroMacro();
    }
  }
}

template < typename FixedImageType, typename MovingImageType, typename TransformType, typename SpecificInitializerType,
           typename DoCenteredInitializationMetricType >
                          ...
                          typename DoCenteredInitializationMetricType::Pointer & CostMetricObject )
{
  ...
  else if ( initializeTransformMode == "useCenterOfROIAlign" || initializeTransformMode == "useCenterOfHeadAlign" )
  {
    ...
    CostMetricObject->Initialize();
    {
        ...
        for ( double HA = -HARange * one_degree; HA <= HARange * one_degree; HA += HAStepSize )
        {
          ...
          for ( double PA = -PARange * one_degree; PA <= PARange * one_degree; PA += PAStepSize )
          {
            ...
            if ( 0 )
            {
              ...
              char filename[300];
              sprintf( filename,
                       "%05.2f_%05.2f_%05.2f.nii.gz",
                       ( currentEulerAngles3D->GetParameters()[2] ) * 180 / itk::Math::pi,
                       ( currentEulerAngles3D->GetParameters()[0] ) * 180 / itk::Math::pi,
                       current_cc );
...
#ifdef DEBUGGING_PRINT_IMAGES
      {
        std::cout << "FINAL: quick search "
                  ...
                  << " PA= " << ( bestEulerAngles3D->GetParameters()[0] ) * 180.0 / itk::Math::pi << " cc=" << max_cc
                  << std::endl;
      }
#endif
    }
...
#ifdef DEBUGGING_PRINT_IMAGES
    {
      ...
      char filename[300];
      sprintf( filename,
               "FINAL_%05.2f_%05.2f_%05.2f.nii.gz",
               ( bestEulerAngles3D->GetParameters()[2] ) * 180 / itk::Math::pi,
               ( bestEulerAngles3D->GetParameters()[0] ) * 180 / itk::Math::pi,
               max_cc );
  ...
  {
    itkGenericExceptionMacro( << "FAILURE:  Improper mode for initializeTransformMode: " << initializeTransformMode );
  }
  std::cout << "Initializing transform with " << initializeTransformMode << " to " << std::endl;
...
BRAINSFitHelperTemplate< FixedImageType, MovingImageType >::FitCommonCode(
  int numberOfIterations, double minimumStepLength, typename CompositeTransformType::Pointer & initialITKTransform )
{
  // FitCommonCode
...
BRAINSFitHelperTemplate< FixedImageType, MovingImageType >::Update( void )
{
  ...
  for ( unsigned int currentTransformIndex = 0; currentTransformIndex < m_TransformType.size();
  ...
  // Use CenteredVersorTranformInitializer
  {
    ...
    TransformType::Pointer initialITKTransform =
      DoCenteredInitialization< FixedImageType, MovingImageType, TransformType, InitializerType, MultiMetricType >(
        m_FixedVolume,
        m_MovingVolume,
        m_FixedBinaryVolume,
        ...
        currentTransformIndex++ )
  {
    const std::string currentTransformType( m_TransformType[currentTransformIndex] );
    std::cout << "\n\n\n=============================== "
              << "ITKv4 Registration: Starting Transform Estimations for " << currentTransformType << "("
    ...
    if ( currentTransformType == "Rigid" )
    {
      ...
      if ( m_CurrentGenericTransform
             .IsNotNull() ) // When null, m_CurrentGenericTransform will be initialized by identity.
      {
         ...
         * NOTE2: If the transform Type of the current level is linear,
                  the m_CurrentGenericTransform should contain only one transform as we collapse all linear transforms
         together.
         */
        ...
        try
        {
          ...
          else
          {
            std::cout << "Unsupported initial transform file -- TransformBase first transform typestring, "
                      << transformFileType << " not equal to required type VersorRigid3DTransform" << std::endl;
            return;
          }
        }
      ...
      this->FitCommonCode< TransformType, OptimizerType, MetricType >( localNumberOfIterations[currentTransformIndex],
                                                                       localMinimumStepLength[currentTransformIndex],
                                                                       this->m_CurrentGenericTransform );
      // NOW, after running the above function, the m_CurrentGenericTransform contains the integration of initial
      // transform and rigid registration results.
      ///////////////////////
    }
    else if ( currentTransformType == "ScaleVersor3D" )
    {
      ...
      if ( m_CurrentGenericTransform.IsNotNull() )
      {
        ...
        try
        {
          ...
          else
          {
            std::cout << "Unsupported initial transform file -- TransformBase first transform typestring, "
                      << transformFileType
                      << " not equal to required type VersorRigid3DTransform OR ScaleVersor3DTransform" << std::endl;
            return;
          }
        }
        catch ( itk::ExceptionObject & excp )
      ...
      this->FitCommonCode< TransformType, OptimizerType, MetricType >( localNumberOfIterations[currentTransformIndex],
                                                                       ...
                                                                       this->m_CurrentGenericTransform );
      // NOW, after running the above function, the m_CurrentGenericTransform contains the integration of initial
      // transform and ScaleVersor registration results.
      /////////////////////
    }
    else if ( currentTransformType == "ScaleSkewVersor3D" )
    {
      ...
      if ( m_CurrentGenericTransform.IsNotNull() )
      {
        ...
        try
        {
          ...
          {
            std::cout << "Unsupported initial transform file -- TransformBase first transform typestring, "
                      << transformFileType << " not equal to required type VersorRigid3DTransform "
                      << "OR ScaleVersor3DTransform OR ScaleSkewVersor3DTransform" << std::endl;
            return;
          }
        }
        catch ( itk::ExceptionObject & excp )
      ...
      this->FitCommonCode< TransformType, OptimizerType, MetricType >( localNumberOfIterations[currentTransformIndex],
                                                                       ...
                                                                       this->m_CurrentGenericTransform );
      // NOW, after running the above function, the m_CurrentGenericTransform contains the integration of initial
      // transform and ScaleSkew registration results that is an "Affine" transform.
      /////////////////////
    }
    else if ( currentTransformType == "Affine" )
    {
      ...
      if ( m_CurrentGenericTransform.IsNotNull() )
      {
        ...
        try
        {
          ...
          {
            std::cout << "Unsupported initial transform file -- TransformBase first transform typestring, "
                      << transformFileType << " not equal to any recognized type VersorRigid3DTransform OR "
      ...
      this->FitCommonCode< TransformType, OptimizerType, MetricType >( localNumberOfIterations[currentTransformIndex],
                                                                       ...
                                                                       this->m_CurrentGenericTransform );
      // NOW, after running the above function, the m_CurrentGenericTransform contains the integration of initial
      // transform and Affine registration results.
      /////////////////////
    }
    else if ( currentTransformType == "BSpline" )
    {
      ...
      if ( m_UseROIBSpline )
      {
        ...
        else if ( m_FixedBinaryVolume.GetPointer() != nullptr )
        {
          ImageMaskSpatialObjectType::Pointer fixedImageMask =
            dynamic_cast< ImageMaskSpatialObjectType * >( m_FixedBinaryVolume.GetPointer() );
...
#  endif
      ...
      std::cout << " - Maximum Number of Iterations     : " << localNumberOfIterations[currentTransformIndex]
                << std::endl
                << std::endl;
      ...
      if ( !this->m_InitializeRegistrationByCurrentGenericTransform )
      {
        if ( this->m_CurrentGenericTransform.IsNotNull() )
        {
          ...
          typename MovingImageType::Pointer warpedMoving1 =
            GenericTransformImage< MovingImageType, MovingImageType, DisplacementFieldType >(
              this->m_MovingVolume,
              this->m_FixedVolume,
          ...
          if ( movingMask.IsNotNull() )
          {
            typename MaskImageType::Pointer warpedMovingMaskImage =
              GenericTransformImage< MaskImageType, MaskImageType, DisplacementFieldType >(
      ...
      try
      {
        std::cout << "*** Running bspline registration (meshSizeAtBaseLevel = " << meshSize << ") ***" << std::endl
                  << std::endl;
        ...
        if ( this->m_SaveState != "" )
        {
          ...
          if ( internalSyNSavedState.IsNotNull() )
          {
                 ...
                 endTransformFileType == "DisplacementFieldTransform" )
            {
              ...
              if ( oneToEndTransform->GetInverseDisplacementField() && endTransform->GetInverseDisplacementField() )
              {
...
#  endif
    ...
    {
      itkGenericExceptionMacro( << "Error choosing what kind of transform to fit \"" << currentTransformType << "("
                                << currentTransformIndex + 1 << " of " << m_TransformType.size() << "). " );
    }
  }
  return;
