                                   ...
                                   const itk::Image<unsigned char, 3>::Pointer & inputLabelImage)
{
  ...
  const auto labelConnectedComponentsFilter = FilterType::New();
  ...
  if (openingSize > 0)
  {
    ...
    const auto erode = ErodeType::New();
  ...
  const auto relabel = RelabelType::New();
  ...
  const auto labelThreshold = BinaryThresholdFilter::New();
  ...
  if (finalOpeningSize > 0)
  {
    ...
    const auto finalErode = ErodeType::New();
  ...
  const auto myMult1 = multFilter::New();
  myMult1->SetInput1(safetyImage);
  myMult1->SetInput2(Mask);
  myMult1->Update();

  const auto myMult2 = multFilter::New();
                           ...
                           const itk::Image<unsigned char, 3>::Pointer & inputLabelImage)
{
  ...
  const auto threshold = BinaryThresholdFilter::New();
