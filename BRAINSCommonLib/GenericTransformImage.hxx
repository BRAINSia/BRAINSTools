  ...
  const bool          binaryFlag)
{
  ...
  if (genericTransform.IsNotNull())
  {
    if (interpolationMode == "ResampleInPlace")
    {
      ...
      TransformedImage =
        DoResampleInPlace<InputImageType, OutputImageType>(PrincipalOperandImage.GetPointer(), genericTransform);
    }
    else
    {
  ...
  const bool          binaryFlag)
{
  ...
  // TODO: Modularize GenericTransformImage and DRY out duplicaiton in these two functions.
  typename InputImageType::ConstPointer PrincipalOperandImage;
