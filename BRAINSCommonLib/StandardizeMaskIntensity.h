...
typename ImageType::Pointer
StandardizeMaskIntensity( typename ImageType::Pointer image, typename LabelImageType::Pointer mask, const double lFract,
                          const double uFract, const typename ImageType::PixelType lowerPeggedValue,
                          const typename ImageType::PixelType upperPeggedValue,
                          const typename ImageType::PixelType clipMin, const typename ImageType::PixelType clipMax )
{
  ...
  else
  {
    ...
    typename LabelImageType::Pointer resampledMask = ResampleImageWithIdentityTransform< LabelImageType >(
      "NearestNeighbor", 0, mask.GetPointer(), image.GetPointer() );
