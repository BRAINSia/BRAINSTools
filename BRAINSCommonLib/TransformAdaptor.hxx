...
TransformAdaptor<TCoordinateType, NDimensions, TInputImage>::TransformAdaptor()
  : m_FixedImage(NULL)
  , m_MovingImage(NULL)
...
template <typename TCoordinateType, unsigned int NDimensions, typename TInputImage>
void
TransformAdaptor<TCoordinateType, NDimensions, TInputImage>::ExecuteInput()
{
  if (GetInputAffineTransformFilename().size() != 0)
  {
    std::cout << "Initial Transform :" << GetInputAffineTransformFilename() << std::endl;
    // Only read if InputAffineTransformFilename was set.

    if (GetInputAffineTransformFilename().find(".air16") != std::string::npos)
    {
      SetInputAffineTransform(ReadAir16File(GetInputAffineTransformFilename().c_str()));
      EstablishCrossOverSystemForAir16();
      ConvertInputAffineToITKAffine();
    }
    else if (GetInputAffineTransformFilename().find(".xfrm") != std::string::npos)
    {
      SetInputAffineTransform(ReadB2AffineFile(GetInputAffineTransformFilename().c_str()));
      EstablishCrossOverSystemForB2xfrm();
      ConvertInputAffineToITKAffine();
    }
    else
    {
      itkExceptionMacro(<< "Unsupported transform file type, " << GetInputAffineTransformFilename());
    }
  }
...
template <typename TCoordinateType, unsigned int NDimensions, typename TInputImage>
void
TransformAdaptor<TCoordinateType, NDimensions, TInputImage>::ExecuteOutput()
{
  /***************************
  ...
  if (GetOutputAffineTransformFilename().size() != 0)
  {
    ...
    else if (GetOutputAffineTransformFilename().find(".xfrm") != std::string::npos)
    {
      EstablishCrossOverSystemForB2xfrm();
      ConvertITKAffineToOutputAffine();
    }
...
TransformAdaptor<TCoordinateType, NDimensions, TInputImage>::ConvertInputAffineToITKAffine(void)
{
  ...
  GetITKAffineTransform()->Compose(GetCrossOverAffineSystem()->GetInhaleEncodeConversion(), ApplyUpstream);
  GetITKAffineTransform()->Compose(GetInputAffineTransform(), ApplyUpstream);
  GetITKAffineTransform()->Compose(GetCrossOverAffineSystem()->GetInhaleDecodeConversion(), ApplyUpstream);
#ifndef NDEBUG
  std::cout << std::endl << " <]-- Inhaled Shift: " << GetITKAffineTransform()->GetOffset() << std::endl;
  std::cout << " <]-- Inhaled Rotation: " << std::endl << GetITKAffineTransform()->GetMatrix() << std::endl;
#endif
}
...
#ifndef NDEBUG
  std::cout << "Exhaling Shift: " << GetITKAffineTransform()->GetOffset() << std::endl;
#endif

  GetOutputAffineTransform()->Compose(GetCrossOverAffineSystem()->GetExhaleEncodeConversion(), ApplyUpstream);
  GetOutputAffineTransform()->Compose(GetITKAffineTransform(), ApplyUpstream);
  GetOutputAffineTransform()->Compose(GetCrossOverAffineSystem()->GetExhaleDecodeConversion(), ApplyUpstream);

#ifndef NDEBUG
  std::cout << std::endl << " <]-- Exhaled Shift: " << GetOutputAffineTransform()->GetOffset() << std::endl;
  std::cout << " <]-- Exhaled Rotation: " << std::endl << GetOutputAffineTransform()->GetMatrix() << std::endl;
#endif
}
} // namespace itk
