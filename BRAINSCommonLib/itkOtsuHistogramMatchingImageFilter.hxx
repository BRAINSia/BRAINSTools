  ...
  ThreadIdType                  threadId)
{
  ...
  for (int i = 0; !outIter.IsAtEnd(); ++inIter, ++outIter, ++i)
  {
...

    {
      ...
      //      mappedValue=std::max(mappedValue,static_cast<double>(m_SourceMinValue));
      //      mappedValue=std::min(mappedValue,static_cast<double>(m_SourceMaxValue));
...

  {
    ...
    while (!iter.IsAtEnd())
    {
      ...
      if (static_cast<double>(value) >= minValue && static_cast<double>(value) <= maxValue)
      {
        ...
        {
          const auto physicalPoint = image->template TransformIndexToPhysicalPoint<double>(iter.GetIndex());
