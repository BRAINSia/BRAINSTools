  ...
  {
    {
      ...
      {
        const auto currPt = image->template TransformIndexToPhysicalPoint<double>(ROIIter.GetIndex());
...
BRAINSHoughEyeDetector<TInputImage, TOutputImage>::GenerateData()
{
  ...
  const auto physicalEye1 = image->template TransformIndexToPhysicalPoint<double>(indexEye1);
  const auto physicalEye2 = image->template TransformIndexToPhysicalPoint<double>(indexEye2);
