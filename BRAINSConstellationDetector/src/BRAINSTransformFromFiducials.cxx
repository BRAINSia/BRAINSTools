...
computeSymmetricScale( const std::vector< itk::Point< double, 3 > > & fixedPoints,
                       ...
                       const itk::Point< double, 3 > & fixedcenter, const itk::Point< double, 3 > & movingcenter )
{
...
static SimilarityTransformType::Pointer
...
main( int argc, char * argv[] )
{
  ...
  else
  {
    ...
    if ( fixedLandmarksFile != "" || movingLandmarksFile != "" )
    {
      ...
      for ( LocalLandmarksMapType::const_iterator fmapit = fixedLandmarkMap.begin(); fmapit != fixedLandmarkMap.end();
            ++fmapit )
      {
