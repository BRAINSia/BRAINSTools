...
public:
  ...
  SetNormal()
  {
    ...
    AB[1] = B[1] - A[1];
    AB[2] = B[2] - A[2];
    std::cout << "AB::: [" << doubleConvert( AB[0] ) << ", " << doubleConvert( AB[1] ) << ", " << doubleConvert( AB[2] )
              << " ]" << std::endl;
    ...
    AC[1] = C[1] - A[1];
    AC[2] = C[2] - A[2];
    std::cout << "AC::: [" << doubleConvert( AC[0] ) << ", " << doubleConvert( AC[1] ) << ", " << doubleConvert( AC[2] )
              << " ]" << std::endl;
    ...
    std::cout << "Normal:::  After Norm[" << doubleConvert( normal[0] ) << ", " << doubleConvert( normal[1] ) << ", "
              << doubleConvert( normal[2] ) << " ]" << std::endl;
  }
...
void
CutBinaryVolumeByPlaneWithDirection( typename TImageType::Pointer * _imageVolume, ThreeLandmarksForPlane * currentPlane,
                                     const std::string & direction )
{
  using ImageRegionIteratorType = itk::ImageRegionIterator< TImageType >;
  ImageRegionIteratorType it( *_imageVolume, ( *_imageVolume )->GetRequestedRegion() );
...
template < typename TImageType >
void
CutBinaryVolumeByPointWithDirection( typename TImageType::Pointer * _imageVolume, const LandmarkPointType & _landmark,
...
main( int argc, char * argv[] )
{
  ...
  // check input
  if ( inputBinaryVolume.empty() || inputLandmarksFilename.empty() )
  {
  ...
  if ( inputLandmarkNames.size() != setCutDirectionForLandmark.size() )
  {
    std::cout << " Size should match between inputLandmarkNames and"
              << " setCutDirectionForLandmark but " << inputLandmarkNames.size()
        ...
        ++ldmkIt, ++dircIt )
  {
    ...
    std::cout << "currentLdmk:: " << *ldmkIt << "::" << currentLdmk[0] << ", " << currentLdmk[1] << ", "
  ...
  if ( inputLandmarkNamesForObliquePlane.size() != 0 )
  {
    if ( inputLandmarkNamesForObliquePlane.size() != 3 * setCutDirectionForObliquePlane.size() )
    {
      std::cout << "Directional information for plane has to be match "
                << "to the number of plane descriptions.(" << inputLandmarkNamesForObliquePlane.size()
                << "!=" << setCutDirectionForObliquePlane.size() << ")" << std::endl;
      return EXIT_FAILURE;
    }
  }
        ...
        ++inputLdmrIt )
  {
    ...
    for ( unsigned int it = 0; it < 3; ++it ) // make sure we have only three ldmr per plane
    {
      ...
      std::cout << "currentLandmark:: " << *inputLdmrIt << "::" << currentLandmark[0] << ", " << currentLandmark[1]
