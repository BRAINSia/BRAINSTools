...
void
LLSModel ::SetFileName(const std::string & fileName)
{
  m_FileName = fileName;
}
...
void
LLSModel ::WriteMatrix(const std::string & path, const MatrixType & matrix)
{
  hsize_t dims[2];
...
LLSModel ::Write()
{
  ...
  if (this->m_LLSMeans.empty() || this->m_LLSMatrices.empty() || this->m_LLSSearchRadii.empty())
  {
    return -1;
  }
  try
...
void
LLSModel ::SetLLSMeans(const LLSMeansType & llsMeans)
{
  this->m_LLSMeans = llsMeans;
}

const LLSModel::LLSMeansType &
LLSModel ::GetLLSMeans() const
{
  return this->m_LLSMeans;
}

void
LLSModel ::SetLLSMatrices(const LLSMatricesType & llsMatrices)
{
  this->m_LLSMatrices = llsMatrices;
}

const LLSModel::LLSMatricesType &
LLSModel ::GetLLSMatrices() const
{
  return this->m_LLSMatrices;
}

void
LLSModel ::SetSearchRadii(const LLSSearchRadiiType & llsSearchRadii)
{
  this->m_LLSSearchRadii = llsSearchRadii;
}

const LLSModel::LLSSearchRadiiType &
LLSModel ::GetSearchRadii() const
{
