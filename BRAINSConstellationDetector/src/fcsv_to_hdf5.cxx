...
static std::string
sanitize( const std::string & landmarkname )
{
...
read_csv( const std::string & file )
{
...
static std::string
get_subjectid( const std::string & content )
{
...
static void
print_arguments_for_user( const std::string & file_glob_string, const std::string & outfile,
...
SubjectFilenameVector
get_subject_filename_tuples( const std::string & file_glob_string )
{
  wordexp_t p;
...
static FileToLandmarksMapType
get_allFileToLandmarkMap( const std::vector< std::pair< std::string, std::string > > & subjects )
{
...
get_landmark_types( const std::string & landmark_types_list_file )
{
  ...
  for ( std::vector< std::vector< std::string > >::const_iterator iter = csv_data.begin(); iter != csv_data.end();
        ++iter )
  {
    // Check for comment
...
static void
WriteHDFStringList( H5::H5File & file, const char * const name, const std::vector< std::string > & stringList )
{
...
main( int argc, char * argv[] )
{
  ...
  */
  ...
  for ( unsigned int i = 0; i < numNewLandmarks; i++ )
  {
    if ( i > 0 )
    {
      vnl_matrix< double > lmkVec = ( ( byClassLandmarkMatrix["newLandmarks"][i - 1].second ) -
    ...
    */
    ...
    for ( unsigned int j = 0; j < numDatasets; j++ )
    {
      for ( unsigned int k = 1; k < ( numBaseLandmarks + i ); k++ )
      {
        ...
        Xi_t.update(
          ( xk_t.extract( 1, xk_t.columns(), j, 0 ).transpose() - x1_t.extract( 1, x1_t.columns(), j, 0 ).transpose() ),
          ( k - 1 ) * dim,
          0 );
      }
      ...
      lmk_est[j].update(
        ( x1_t.extract( 1, x1_t.columns(), j, 0 ).transpose() + ( M[i].transpose() * Xi_demeaned_t ) ), 0, i );
      lmk_groundTruth = byClassLandmarkMatrix["newLandmarks"][i].second;
      err[j].update( ( lmk_est[j].extract( lmk_est[j].rows(), 1, 0, i ) -
                       lmk_groundTruth.extract( 1, lmk_groundTruth.columns(), j, 0 ).transpose() ),
                     0,
                     i );
    }
  }
