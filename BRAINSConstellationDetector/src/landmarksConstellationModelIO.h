...
////
inline void
defineTemplateIndexLocations(const float                                      r,
                             const float                                      h,
...
public:
  ...
  void
  SetRPtoCECMean(const SImageType::PointType::VectorType & RPtoCECMean)
  {
    this->m_RPtoCECMean = RPtoCECMean;
  }

  void
  SetRPPC_to_RPAC_angleMean(float RPPC_to_RPAC_angleMean)
  {
    this->m_RPPC_to_RPAC_angleMean = RPPC_to_RPAC_angleMean;
  }

  void
  SetRPAC_over_RPPCMean(float RPAC_over_RPPCMean)
  {
  ...
  const SImageType::PointType::VectorType &
  GetRPtoCECMean() const
  {
    return this->m_RPtoCECMean;
  }

  float
  GetRPPC_to_RPAC_angleMean() const
  {
    return this->m_RPPC_to_RPAC_angleMean;
  }

  float
  GetRPAC_over_RPPCMean() const
  {
   ...
   */
  void
  ComputeAllMeans(Float2DVectorType & output, const Float3DVectorType & input)
  {
  ...
  AccessTemplate(std::string name, const unsigned int indexDataSet, const unsigned int indexAngle)
  {
    ...
    {
      itkGenericExceptionMacro(<< "Attempt to access an undifined landmark template for " << name);
    }
  ...
  const Float2DVectorType &
  GetTemplateMeans(const std::string & name)
  {
    return this->m_TemplateMeans[name];
  }

  void
  WriteModelFile(const std::string & filename)
  {
    //
    //
    // //////////////////////////////////////////////////////////////////////////
    ...
    try
    {
      ...
      for (it2 = this->m_TemplateMeansComputed.begin(); it2 != this->m_TemplateMeansComputed.end(); ++it2)
      {
        ComputeAllMeans(this->m_TemplateMeans[it2->first], this->m_Templates[it2->first]);
  ...
  PrintHeaderInfo()
  {
    ...
    //
    // //////////////////////////////////////////////////////////////////////////
    std::cout << "SearchboxDims"
              << ": " << this->GetSearchboxDims() << std::endl;
    std::cout << "ResolutionUnits"
              << ": " << this->GetResolutionUnits() << std::endl;
    std::cout << "NumDataSets"
              << ": " << this->GetNumDataSets() << std::endl;
    std::cout << "NumRotationSteps"
              << ": " << this->GetNumRotationSteps() << std::endl;
  ...
  void
  ReadModelFile(const std::string & filename)
  {
    //
    //
    // //////////////////////////////////////////////////////////////////////////
  ...
  public:
    ...
    debugImageDescriptor(Float2DVectorType & _mean, float _r, float _h, const char * _debugImageName)
      ...
      , r(_r)
      , h(_h)
      , debugImageName(_debugImageName)
    {
  ...
  WritedebugMeanImages(std::string name)
  {
    ...
    for (int j = 0; meanIt != DID.mean.end(); ++meanIt, j++)
    {
      ...
      region.SetSize(0, static_cast<FloatImageRegion::SizeValueType>(DID.h + 1));
      region.SetSize(1, static_cast<FloatImageRegion::SizeValueType>(2.0 * DID.r) + 1);
      region.SetSize(2, static_cast<FloatImageRegion::SizeValueType>(2.0 * DID.r) + 1);
  ...
  void
  InitializeModel(bool CreatingModel)
  {
    //
    //
    // ///////////////////////////////////////////////////////////////////////////////
    ...
    for (it2 = this->m_TemplateMeansComputed.begin(); it2 != this->m_TemplateMeansComputed.end(); ++it2)
    {
      defineTemplateIndexLocations(
        this->GetRadius(it2->first), this->GetHeight(it2->first), this->m_VectorIndexLocations[it2->first]);
      if (CreatingModel)
      {
      ...
      else // make room for reading in template means result
      {
        ...
        {
          this->m_TemplateMeans[it2->first][i].resize(this->m_VectorIndexLocations[it2->first].size());
        }
      }
    }
  }
  ...
  void
  CopyFromModelDefinition(const landmarksConstellationTrainingDefinitionIO & mDef)
  {
  ...
  bool
  NE(const std::string & label, const Float2DVectorType & a, const Float2DVectorType & b)
  {
    itk::NumberToString<double> doubleToString;
    ...
    for (unsigned i = 0; i < a.size(); i++)
    {
      for (unsigned j = 0; j < a[i].size(); j++)
      {
        if (NE(a[i][j], b[i][j]))
        {
          std::cerr << label << " a[" << i << "] = " << doubleToString(a[i][j]) << "b[" << j
  ...
  operator==(Self & other)
  {
    if ((NE(this->m_SearchboxDims, other.m_SearchboxDims)) || (NE(this->m_ResolutionUnits, other.m_ResolutionUnits)) ||
        (NE(this->m_NumDataSets, other.m_NumDataSets)) || (NE(this->m_NumRotationSteps, other.m_NumRotationSteps)) ||
        (NE(this->m_RPPC_to_RPAC_angleMean, other.m_RPPC_to_RPAC_angleMean)) ||
        (NE(this->m_RPAC_over_RPPCMean, other.m_RPAC_over_RPPCMean)) ||
        (NE(this->m_CMtoRPMean[0], other.m_CMtoRPMean[0])) || (NE(this->m_CMtoRPMean[1], other.m_CMtoRPMean[1])) ||
    ...
    for (it2 = this->m_TemplateMeansComputed.begin(); it2 != this->m_TemplateMeansComputed.end(); ++it2)
    {
      if ((NE(this->GetRadius(it2->first), other.GetRadius(it2->first))) ||
          ...
          (NE(it2->first + " template mean", this->m_TemplateMeans[it2->first], other.m_TemplateMeans[it2->first])))
      {
        return false;
      }
    }
...
private:
...
#ifdef __USE_OFFSET_DEBUGGING_CODE__
  void
  debugOffset(std::ifstream * f, const std::string & msg, long int x)
  {
  ...
  void
  debugOffset(std::ofstream * f, const std::string & msg, long int x)
  {
    std::ostream::pos_type offset(-1);
