 ...
 *
 *=========================================================================*/
#ifndef itkDivergenceImageFilter_h
#define itkDivergenceImageFilter_h
 ...
 *
 * \brief Computes a scalar divergence multiple channel (pixels are vectors) input.
 *
 * \par Overview
 * This filter has two calculation modes.  The first (default) mode calculates
 * divergence as the difference between the largest two eigenvalues in a
 ...
 * found to be.
 *
 * The second, more heuristic, calculation mode finds divergence as the
 * the sum of the individual weighted vector component
 * derivatives. That is, \f$ \mathbf{div} = \left(
 * \sum_{i=0}^n \sum_{j=0}^m \frac{\delta * \phi_j}{\delta \mathbf{x}_{i}}
 * \right) \f$, where \f$\phi_j\f$  is the \f$j^{\mathbf{th}}\f$
 ...
 * of TRealType is PrecisionType.  For extra precision, you may safely change this
 * parameter to double.
 *
 ...
 *
 * The template parameter TRealType must be floating point (PrecisionType or double) or
...
public:
  ...
  itkOverrideGetNameOfClassMacro(DivergenceImageFilter);
   ...
   * DivergenceImageFilter needs to provide an implementation
