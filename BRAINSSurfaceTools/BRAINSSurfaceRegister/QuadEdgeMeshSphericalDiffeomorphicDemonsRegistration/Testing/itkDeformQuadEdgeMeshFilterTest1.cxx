...
int main( int argc, char *argv[] )
{
  ...
  typedef itk::DeformQuadEdgeMeshFilter<
      FixedMeshType, FixedMeshType, PointSetType> DeformFilterType;

  DeformFilterType::Pointer deformFilter =
    DeformFilterType::New();

  deformFilter->SetInput( referenceReader->GetOutput() );
  deformFilter->SetDestinationPoints( demonsFilter->GetFinalDestinationPoints() );
  deformFilter->SetReferenceMesh( fixedReader->GetOutput() );

  deformFilter->SetSphereRadius( 100.0 );
  deformFilter->SetSphereCenter( center0 );
