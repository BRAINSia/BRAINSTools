/*=========================================================================
  ...
  Module:    $RCSfile: itkNodeVectorJacobianCalculatorTest1.cxx,v $
  Language:  C++
...
int main( int argc, char *argv[] )
{
  ...
  typedef FixedMeshType::PointsContainer FixedPointContainerType;

  FixedPointContainerType::Pointer fixedPoints = FixedPointContainerType::New();

  FixedPointDataContainerType::Pointer arraysPerNode = FixedPointDataContainerType::New();

  arraysPerNode->Reserve( fixedReader->GetOutput()->GetNumberOfPoints() );

  // FIXME: Initialize arraysPerNode by copying values from the fixed point coordinates
  std::cout << "Arrays per node " << arraysPerNode.GetPointer() << std::endl;

  gradientCalculator->SetVectorContainer( arraysPerNode );

  if( gradientCalculator->GetVectorContainer() != arraysPerNode )
    {
    std::cerr << "Error in SetDataContainer()/GetDataContainer() " << std::endl;
    return EXIT_FAILURE;
    }

  std::cout << "test 1 gradientCalculator->GetVectorContainer() "
            << gradientCalculator->GetVectorContainer() << "\n";
