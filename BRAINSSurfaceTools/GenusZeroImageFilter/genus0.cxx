...
static int get_cc(unsigned char *_zpic, int *_que, int *_status, size_t *dims, int *ac, int _connectivity)
{
  ...
  if ( _connectivity == 18 ) {c = 18; _offs = _nbrs18; }
  else {c = 6; _offs = nbrs6; }
      ...
      for ( i = 0; i < (int)dims[0]; i++ )
        ...
        if ( _zpic[w] == 1 ) /* not processed yet */
          ...
          for ( h = 0; h < c; h++ ) /* add nbrs to que */
            {
            if ( _zpic[woffsh = w + _offs[h]] == 1 ) { _zpic[que[_que_len++] = woffsh] = 2; }}
          ...
          while ( _que_pos < _que_len )
            ...
            for ( h = 0; h < c; h++ ) /* add nbrs to que */
              {
              if ( _zpic[woffsh = w0 + _offs[h]] == 1 ) { _zpic[_que[_que_len++] = woffsh] = 2; }}
            }
          }
        w++;
  ...
  for ( i = 1; i <= groups; i++ )
    {
    if ( g_counts[i] > w ) {w = g_counts[i]; h = i; }}
  for ( i = 0; i < vox; i++ )
    {
  ...
  for ( k = h = 0; k < (int)dims[2]; k++ )
    ...
    for ( j = 0; j < (int)dims[1]; j++ )
      ...
      for ( i = 0; i < (int)dims[0]; i++ )
        ...
        if ( _zpic[h] )
          {
          if ( _autocrop[0][0] > i ) {_autocrop[0][0] = i; } if ( _autocrop[0][1] < i ) {_autocrop[0][1] = i; }
          if ( _autocrop[1][0] > j ) {_autocrop[1][0] = j; } if ( _autocrop[1][1] < j ) {_autocrop[1][1] = j; }
          if ( _autocrop[2][0] > k ) {_autocrop[2][0] = k; } if ( _autocrop[2][1] < k ) {_autocrop[2][1] = k; }
          }
        h++;
        }
      }
...
static int set_up(genus0parameters *g0)
{
  ...
  for ( i = 0; i < 3; i++ )
    {
    if ( ( g0->extraijkscale )[i] <= 0.0 ) { ( g0->extraijkscale )[i] = 1.0; }}
  for ( i = 0; i < 3; i++ )
    {
...
#include "tricases.h"
  ...
  *Tri_count = *Vert_count = 0;
  ...
  if ( _status == NULL ) {error_msg("Memory error.\n", __LINE__); return( 1 ); }
  ...
  for ( i = 0; i < 256; i++ ) /* calculate number of tris, new verts for each
    ...
    for ( j = 0; j < 19; j++ )
      ...
      for ( w = 0; w < 3; w++ )
        {
        if ( tricases[i][j] == hep2[w] ) { hep[w] = 1; }}
      }
    tc[i] = k / 3;
...
static int big_component(int *Tris, float *Verts, int *Vert_count, int *Tri_count)
{
      ...
      while ( _que_pos < _que_len )                   /* while que not empty */
        ...
        for ( i = v_ran[qqp]; i < v_ran[qqp + 1]; i++ )
          ...
          for ( j = 0; j < 3; j++ )
            ...
            if ( component[k] == 0 ) {component[_que[_que_len++] = k] = _comp_count; }
            }
          }
        _que_pos++;
        }
  ...
  if ( _comp_count > 1 )
    ...
    for ( i = 1; i <= _comp_count; i++ )
      {
      if ( k < c_count[i] ) {j = i; k = c_count[i]; }}
    Gfree(c_count);
