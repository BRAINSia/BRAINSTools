...
static int sub2ind(int horiz, int vert, int depth, int _img_horiz, int _img_vert)
{                                                            /* convert
                                                                 subscripts to
                                                                 linear index */
...
static int get_cc(unsigned char *_zpic, int *_que, int *_status, size_t *dims, int *ac, int _connectivity)
{
  ...
  for( i = 0; i < vox; i++ )
    {
    _zpic[i] = ( _status[i] == h ); _status[i] = 0;
    }
  for( k = 0; k < 3; k++ )
    {
    _autocrop[k][0] = dims[k]; _autocrop[k][1] = 0;
    }
...
static int set_up(genus0parameters *g0)
{
  ...
  for( k = 0; k < 3; k++ )
    {
    autocrop[k][0] = ( g0->dims )[k]; autocrop[k][1] = 0;
    }
...
static int truecm(int st) /* return true component, set cm[st] to true comp */
{
  ...
  if( cm[st] != st )
    ...
    while( cm[s0] != st )
      {
      s1 = cm[s0]; cm[s0] = st; s0 = s1;             /*std::cout << "While loop
                                                       2 " << s0 <<
                                                       std::endl;*/
      }
    }
...
static int GetSurf(unsigned char *J, unsigned char val, int *dims, int _connectivity,
...
#include "tricases.h"
  ...
  *Tri_count = *Vert_count = 0;
  ...
  _offs[0] = 0; _offs[1] = 1; _offs[2] = _img_horiz; _offs[3] = ih1;
  for( i = 4; i < 8; i++ )
    {
  ...
  for( i = 4; i < 8; i++ )
    {
    coffs[i] = coffs[i - 4]; ctype[i] = ctype[i - 4];
    }
          ...
          for( w = 0; w < u; w += 3 )  /* for each triangle */
            ...
            for( v = 0; v < 3; v++ )  /* for each vert of triangle */
              ...
              tris[tri_count1 + tcm[v]] = v_idx[planeoff[planeidx[tsvw]] + pc
                                                + coffs[tsvw] + cellplanem[ctype[tsvw]]]; /*
                                                                                            whew!
                                                                                            */
              }
            tri_count1++;
...
static int save_image(genus0parameters *g0)
{
  ...
  if( !( g0->return_surface ) )  /* don't return surface if they didn't want it
                                   */
    {
    Gfree(g0->vertices); g0->vertices = nullptr;
    Gfree(g0->triangles); g0->triangles = nullptr;
    }
