/*=========================================================================
  ...
  Program:   vtkITK
  Module:    $HeadURL: http://svn.slicer.org/Slicer3/trunk/Libs/vtkITK/vtkITKArchetypeImageSeriesScalarReader.cxx $
...
#define vtkITKExecuteDataFromSeries(typeN, type)                        \
  ...
  ( dynamic_cast<vtkImageData *>( output ) )->GetPointData()->GetScalars()->SetVoidArray( \
    ptr,                                                                \
    PixelContainer ## typeN->                                           \
    Size(),                                                             \
    0);                                                                 \
...
#define vtkITKExecuteDataFromFile(typeN, type)                          \
  ...
  if ( this->FileNames.size() == 1 )
    ...
    if ( this->GetNumberOfComponents() == 1 )
      ...
      {
      vtkErrorMacro(
        << "UpdateFromFile: Unsupported number of components (only 1 allowed): " << this->GetNumberOfComponents() );
      }
    }
  else
    {
...
void vtkITKArchetypeImageSeriesScalarReader::ReadProgressCallback(itk::ProcessObject *obj,
