...
int main(int argc, char *argv[])
{
  ...
  ReadVolume<VectorImageType>(img, inputName);
  ...
  ReadVolume<Image3DType>(img3D, inputName);
  WriteVolume<Image3DType>(img3D, outputName3D);

  ReadVolume<Image4DType>(img4D, outputName);
  std::cout << "output directions" << std::endl
            << img4D->GetDirection() << std::endl;

  ReadVolume<Image3DType>(img3D, outputName3D);
  std::cout << "output directions" << std::endl
