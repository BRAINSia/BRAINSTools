...
QImageDisplay::SetImage(const std::string & fileName)
{
  ReadImageType::Pointer readImage
    = itkUtil::ReadImageAndOrient<ReadImageType>
        (fileName,
        itk::SpatialOrientation::
        ITK_COORDINATE_ORIENTATION_RAI);
  ImageType::Pointer image =
    DebugImageViewerUtil::ScaleAndCast<ReadImageType, ImageType>
      (readImage, 0, 255);

  this->SetImage(image);
}

QImageDisplay::QImageDisplay(QWidget *parent) : QWidget(parent),
  m_ImageViewer(0),
  m_Image(0),
  m_Slider(0)
{
  ...
  connect(combo, SIGNAL(currentIndexChanged(const QString &) ),
          this, SLOT(viewChanged(const QString &) ) );

  this->m_Slider = new QSlider(Qt::Vertical, this);
  this->m_Slider->setSingleStep(1);
  connect(this->m_Slider, SIGNAL(valueChanged(int) ),
          this->m_ImageViewer, SLOT(SetSlice(int) ) );

  layout->addWidget(this->m_ImageViewer,
                    0, 0, 5, 5);
