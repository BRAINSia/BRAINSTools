...

  {
    ...
    if (gradientdirections != 0)
    {
      ...
      if (numGradients > 0)
      {
        ...
        for (unsigned i = 0; i < numGradients; ++i)
        {
          header << "DWMRI_gradient_" << std::setw(4) << std::setfill('0') << i << ":=" << DoubleConvert(gradients[i])
...
itkSaveWithMetaData(int nrhs, const mxArray * prhs[])
{
  ...
  {
    snprintf(
      errBuff, NRRD_MAX_ERROR_MSG_SIZE, "%s: sorry, can't handle type %s", me, mxGetClassName(msm.GetField("data")));
    mexErrMsgTxt(errBuff);
  }
