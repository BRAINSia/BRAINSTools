...
public:
   ...
   */
  bool
  isValidField(const std::string & name) const
  {
  ...
  bool
  structHasField(const std::string & name) const
  {
  ...
  isValidStruct() const
  {
    ...
    {
      snprintf(
        m_errBuff, NRRD_MAX_ERROR_MSG_SIZE, "ERROR: Sorry, can't handle type %s", mxGetClassName(GetField("data")));
      mexErrMsgTxt(m_errBuff);
      return false;
    }
    return true;
...
#endif
  const mwSize *
  GetDimensions(const std::string & name) const
  {
    return mxGetDimensions(this->GetField(name));
  }

  unsigned int
  GetNumberOfDimensions(const std::string & name) const
  {
...
private:
  int
  typeMtoN(const mxClassID mtype) const
  {
    int ntype;
